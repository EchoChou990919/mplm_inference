...
from esm.utils.generation import (
    ...
    iterative_sampling_multitrack_tokens,
)
...
class ESM3(nn.Module, ESM3InferenceClient):
    ...
    def batch_generate(
        ...
        if isinstance(inputs[0], ESMProtein):
            ...
            # ======
            # NOTE: added multitrack generation support
            if isinstance(configs[0], MultiTrackGenerationConfig):
                return iterative_sampling_multitrack_tokens(
                    self, 
                    inputs, 
                    configs, 
                    self.tokenizers
                )
            elif isinstance(configs[0], GenerationConfig):
