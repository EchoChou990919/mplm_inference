...
class GenerationConfig:
    ...
    # "stochastic" will unmask tokens based on the entropy plus some gumbel noise, so it's a stochastic version of entropy-based unmasking.
    strategy: str = attr.field(
        validator=attr.validators.in_(["random", "entropy", "stochastic"]), default="random" # NOTE: added "stochastic" strategy
    )
    ...
    # ======
    # NOTE: added allow_remask for DPLM2-style reconsideration
    # If True, allows remasking of already-decoded positions and reconsidering them in subsequent steps.
    # If False (default), only samples positions that are currently masked (standard iterative sampling).
    allow_remask: bool = False
    # ======

    # ======
    # NOTE: added CFG (Classifier-Free Guidance) settings
    # cfg_scale: guidance strength. 0.0 means no guidance (standard sampling).
    # cfg_mode options (for single-track generation via GenerationConfig):
    #   "drop_condition" — masks all non-target tracks; general purpose.
    #   "full_mask" — masks all tracks including the target; general purpose.
    #   "drop_motif" — masks motif positions across all tracks; for scaffolding tasks.
    # Note: "cross_modal" is NOT supported here (single-track); use GenerationTrackConfig in cogen mode.
    cfg_scale: float = 0.0
    cfg_mode: str = "drop_condition"
    # ======
...
# ======
# NOTE: added MultiTrackGenerationConfig and GenerationTrackConfig
# It's almostly a copy of GenerationConfig, 
# except the 'track', 'condition_on_coordinates_only', 'only_compute_backbone_rmsd' fields are removed. 
@define
class GenerationTrackConfig:
    """Configuration for a single track in multi-track generation.
    
    This class specifies how to generate a particular track (e.g., sequence, structure, etc.)
    with its own independent sampling parameters.
    """
    # Controls the number of tokens to unmask during each round of iterative generation.
    schedule: str = attr.field(
        validator=attr.validators.in_(["cosine", "linear"]), default="cosine"
    ...
    # "entropy" will unmask the tokens with the lowest logit entropy first.
    strategy: str = attr.field(
        validator=attr.validators.in_(["random", "entropy", "stochastic"]), default="random"
    )
    # Setting default to 20, as there is diminishing return for decoding steps more than 20.
    num_steps: int = 20
    temperature: float = 1.0
    ...
    # ======
    # NOTE: added CFG (Classifier-Free Guidance) settings
    # cfg_mode options (for multi-track co-generation via GenerationTrackConfig):
    #   "drop_condition" — masks all non-target tracks; general purpose.
    #   "full_mask" — masks all tracks; general purpose.
    #   "drop_motif" — masks motif positions across all tracks; for scaffolding cogen.
    #   "cross_modal" — builds separate unconds per modality (seq vs struct),
    #                    3×batch forward; for scaffolding cogen and unconditional cogen tasks.
    cfg_scale: float = 0.0
    cfg_mode: str = "drop_condition"
    # ======
...
@define
class MultiTrackGenerationConfig:
    """Configuration for multi-track iterative generation.
    
    This class allows generating multiple tracks (sequence, structure, secondary_structure, sasa, function)
    simultaneously in a single forward pass, with independent sampling parameters for each track.
    
    Each track can have:
    - Independent noise schedule (cosine/linear)
    - Independent sampling strategy (random/entropy/stochastic)
    - Independent number of decoding steps
    - Independent temperature annealing settings
    - Track-specific invalid tokens
    ...
    # Dictionary mapping track names to their generation configs
    tracks: dict[str, GenerationTrackConfig] = attr.field(factory=dict)
    
    # Global settings applicable to all tracks
    condition_on_coordinates_only: bool = True
    only_compute_backbone_rmsd: bool = False

    def __attrs_post_init__(self):
        """Validate track names and configuration."""
        valid_tracks = {
            "sequence",
            "structure",
            "secondary_structure",
        ...
        if not self.tracks:
            raise ValueError("At least one track must be specified in tracks dictionary")
        
        for track_name in self.tracks.keys():
            if track_name not in valid_tracks:
                raise ValueError(
                    f"Invalid track name '{track_name}'. "
                    f"Valid tracks are: {', '.join(sorted(valid_tracks))}"
                )
    ...
    def add_track(
        ...
        Args:
            track_name: Name of the track (sequence, structure, secondary_structure, sasa, function)
            schedule: Noise schedule ("cosine" or "linear")
            strategy: Sampling strategy ("random", "entropy", or "stochastic")
            num_steps: Number of decoding steps
            temperature: Sampling temperature
            temperature_annealing: Whether to anneal temperature during generation
            top_p: Top-p sampling parameter
            invalid_ids: List of invalid token IDs to exclude from sampling
        """
        valid_tracks = {
            "sequence",
            "structure",
        ...
        self.tracks[track_name] = GenerationTrackConfig(
            schedule=schedule,
            strategy=strategy,
            num_steps=num_steps,
    ...
    def remove_track(self, track_name: str) -> None:
        """Remove a track from the configuration.
        
        Args:
            track_name: Name of the track to remove
            
        Raises:
            ValueError: If track doesn't exist or is the last remaining track
        """
        if track_name not in self.tracks:
            raise ValueError(f"Track '{track_name}' not found in configuration")
        
        if len(self.tracks) == 1:
            raise ValueError("Cannot remove the last track. At least one track must be configured.")
        
        del self.tracks[track_name]

    def get_track(self, track_name: str) -> GenerationTrackConfig | None:
        """Get the configuration for a specific track.
        
        Args:
            track_name: Name of the track
            
        Returns:
            GenerationTrackConfig for the track, or None if not found
        """
        return self.tracks.get(track_name)

    def get_max_num_steps(self) -> int:
        """Get the maximum number of steps across all tracks.
        
        Returns:
            Maximum num_steps value among all configured tracks
        """
        return max((config.num_steps for config in self.tracks.values()), default=20)

    def get_all_tracks(self) -> list[str]:
        """Get list of all configured track names.
        
        Returns:
            Sorted list of track names
        """
        return sorted(self.tracks.keys())

    def use_entropy_based_unmasking_strategy(self) -> None:
        """Use entropy based unmasking strategy for all tracks."""
        for config in self.tracks.values():
            config.use_entropy_based_unmasking_strategy()

    def use_generative_unmasking_strategy(self) -> None:
        """Use generative unmasking strategy for all tracks."""
        for config in self.tracks.values():
            config.use_generative_unmasking_strategy()

    def to_dict(self) -> dict:
        """Convert configuration to dictionary format for logging/debugging.
        ...
        return {
            "tracks": {
                name: {
                    ...
                    "top_p": config.top_p,
                    "invalid_ids": list(config.invalid_ids),
                }
                for name, config in self.tracks.items()
            },
            "condition_on_coordinates_only": self.condition_on_coordinates_only,
            "only_compute_backbone_rmsd": self.only_compute_backbone_rmsd,
        }
# ======
