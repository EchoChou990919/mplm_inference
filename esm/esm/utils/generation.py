# NOTE: This file was modified from original file at ESM3

import os
from typing import Any, Callable, Sequence
from warnings import warn

import attr
import torch
from tqdm import tqdm

from esm.sdk.api import (
    ESM3InferenceClient,
    ESMProtein,
    ESMProteinError,
    ESMProteinTensor,
    ForwardAndSampleOutput,
    ForwardTrackData,
    GenerationConfig,
    LogitsConfig,
    LogitsOutput,
    SamplingConfig,
    SamplingTrackConfig,
    MultiTrackGenerationConfig,
)
from esm.tokenization import EsmTokenizerBase, TokenizerCollectionProtocol
from esm.tokenization.function_tokenizer import InterProQuantizedTokenizer
from esm.utils.constants import esm3 as C
from esm.utils.misc import stack_variable_length_tensors
from esm.utils.noise_schedules import NOISE_SCHEDULE_REGISTRY
from esm.utils.sampling import (
    _BatchedESMProteinTensor,
    get_sampling_mask,
    sample_function_logits,
    sample_logits,
    sample_residue_annotation_logits,
    sample_sasa_logits,
    top_p_logits,
)


def _trim_sequence_tensor_dataclass(o: Any, sequence_len: int):
    """Trim tensors on the sequence dimension.

    This util assume that input tensor class has batch dimension.
    """
    assert attr.has(o.__class__)

    sliced = {}
    for k, v in attr.asdict(o, recurse=False).items():
        if k in ["mean_hidden_state", "mean_embedding"]:
            sliced[k] = v
        elif v is None:
            sliced[k] = None
        elif isinstance(v, torch.Tensor):
            # Trim padding.
            sliced[k] = v[:, :sequence_len]
        elif isinstance(v, tuple) and all(isinstance(t, torch.Tensor) for t in v):
            # Trim padding for a list of tensors
            sliced[k] = [t[:, :sequence_len] for t in v]
        elif attr.has(v.__class__):
            # Recursively slice the child attribute.
            sliced[k] = _trim_sequence_tensor_dataclass(v, sequence_len)
        else:
            # Otherwise, simply copy the entire data bit over.
            sliced[k] = v

    return attr.evolve(o, **sliced)


def _slice_tensor_dataclass(o: Any, i: int, keep_dim: bool = False) -> Any:
    """Take a slice out of any attr defined Tensor objects along the batch dimension.

    Args:
        o: input tensor object to be sliced.
        i: index of the row to be sliced.
        keep_dim: whether to keep the batch dim after slicing.
            For example, given a tensor of shape (5, 8), if keep_dim is True,
            return a sliced tensor of shape (1, 8). Return a tensor of shape
            (8,) instead if keep_dim is False. The default is False.
    """
    assert attr.has(o.__class__)

    sliced = {}
    for k, v in attr.asdict(o, recurse=False).items():
        if v is None:
            sliced[k] = None
        elif isinstance(v, torch.Tensor):
            # Select the i-th row of each tensor.
            row = v.select(0, i)
            if keep_dim:
                row = row.unsqueeze(0)
            sliced[k] = row
        elif attr.has(v.__class__):
            # Recursively slice the child attribute.
            sliced[k] = _slice_tensor_dataclass(v, i, keep_dim)
        else:
            # Otherwise, simply copy the entire data bit over.
            sliced[k] = v

    return attr.evolve(o, **sliced)


def iterative_sampling_raw(
    client: ESM3InferenceClient,
    proteins: list[ESMProtein],
    configs: list[GenerationConfig],
) -> list[ESMProtein | ESMProteinError]:
    # Keep structure tokens
    input_tokens = [client.encode(protein) for protein in proteins]

    output_tokens_list = client.batch_generate(input_tokens, configs)

    raw_proteins: list[ESMProtein | ESMProteinError] = []
    for output_tokens in output_tokens_list:
        if isinstance(output_tokens, ESMProteinTensor):
            raw_proteins.append(client.decode(output_tokens))
        elif isinstance(output_tokens, ESMProteinError):
            raw_proteins.append(output_tokens)
        else:
            raise ValueError(f"Unknown output type {type(output_tokens)}")

    for input_protein, raw_protein, config in zip(proteins, raw_proteins, configs):
        if isinstance(raw_protein, ESMProteinError):
            # If this generation errored out.
            continue
        if config.track not in ["function", "residue_annotations"]:
            # Function and residue annotation encoding/decoding is lossy
            # There is no guarantee that decoding encoded tokens will yield the same input
            raw_protein.function_annotations = input_protein.function_annotations

    return raw_proteins


def _make_masked_inputs(
    track: str, sequence_length: int, tokenizers: TokenizerCollectionProtocol
):
    get_tokenizer: Callable[[str], EsmTokenizerBase] = lambda s: getattr(tokenizers, s)
    has_tokenizer: Callable[[str], bool] = lambda s: hasattr(tokenizers, s)

    if track == "coordinates":
        dims = (sequence_length, 3, 3)
    elif track == "confidence":
        dims = (sequence_length,)
    elif track == "attention_mask":
        dims = (sequence_length,)
    elif track == "function":
        dims = (sequence_length, tokenizers.function.depth)
    elif track == "residue_annotations":
        dims = (sequence_length, C.MAX_RESIDUE_ANNOTATIONS)
    else:
        dims = (sequence_length,)

    if track == "coordinates":
        masked_tokens = torch.full(dims, torch.inf, dtype=torch.float)
    elif track == "confidence":
        # All-mask dummy input for confidence track.
        masked_tokens = torch.full(dims, 0.0)
    elif track == "attention_mask":
        masked_tokens = torch.full(dims, 1, dtype=torch.bool)
    elif has_tokenizer(track):
        masked_tokens = torch.full(
            dims, get_tokenizer(track).mask_token_id, dtype=torch.long
        )
        masked_tokens[0] = get_tokenizer(track).bos_token_id
        masked_tokens[-1] = get_tokenizer(track).eos_token_id
    else:
        # Does not know how to create the dummy all masked input.
        return None

    return masked_tokens


def _stack_protein_tensors(
    input_tokens: list[ESMProteinTensor],
    sequence_lengths: list[int],
    tokenizers: TokenizerCollectionProtocol,
    device: str | torch.device,
) -> _BatchedESMProteinTensor:
    o = _BatchedESMProteinTensor()

    def _maybe_mock_input(fn, t, l):
        if t is not None:
            return t

        # Try create dummy masked input for this prompt.
        t = _make_masked_inputs(fn, l, tokenizers)
        if t is not None:
            t = t.to(device)

        return t

    def _stack_field(fn: str):
        tensors = [getattr(tokens, fn) for tokens in input_tokens]

        # Create all mask mock inputs for any tensors that are None.
        tensors = [
            _maybe_mock_input(fn, t, l) for t, l in zip(tensors, sequence_lengths)
        ]

        # Handle any track that has all None as the input.
        # We can't meaningfully stack tensors in this case, so simply batched
        # them as None in _BatchedESMProteinTensor.
        if all([t is None for t in tensors]):
            setattr(o, fn, None)
            return

        if fn == "coordinates":
            mask_token_id = torch.inf
        else:
            mask_token_id = getattr(tokenizers, fn).pad_token_id

        setattr(
            o,
            fn,
            stack_variable_length_tensors(
                sequences=tensors,  # type: ignore
                constant_value=mask_token_id,
            ),
        )

    for f in attr.fields(ESMProteinTensor):
        # We do not batch potential_sequence_of_concern field.
        if f.name == "potential_sequence_of_concern":
            continue
        _stack_field(f.name)

    return o


def _get_masked_positions(
    track: str, tokens: torch.Tensor, mask_token_id: int
) -> torch.Tensor:
    if track == "function":
        mask = torch.all(tokens == mask_token_id, dim=-1).to(tokens.device)
    else:
        mask = tokens == mask_token_id

    # Should not sample BOS and EOS positions.
    mask[..., 0] = False
    mask[..., -1] = False

    return mask


def _get_iterative_sampling_mask_for_prompt_and_step(
    cur_sampled: _BatchedESMProteinTensor,
    sequence_lengths: torch.Tensor,
    total_to_sample: torch.Tensor,
    step: int,
    entropy: ForwardTrackData,
    config: GenerationConfig,
    tokenizers: TokenizerCollectionProtocol,
    initial_mask: torch.Tensor | None = None, # NOTE (ADDED bt ZY)
) -> torch.Tensor:
    """Get sampling mask based on forward output and config.

    Args:
        cur_sampled: Current sampled tokens
        sequence_lengths: Sequence length for each batch
        total_to_sample: Total number of positions to sample initially
        step: Current decoding step
        entropy: Entropy from forward pass
        config: Generation configuration (includes allow_remask flag)
        tokenizers: Tokenizer collection
        initial_mask: Boolean tensor indicating initially masked positions. 
                     If provided and allow_remask=True, allows remasking and reconsidering any
                     position that was initially masked (DPLM2-style reparameterized decoding).

    Returns:
        Sampling mask and num of positions sampled.
    """
    track_to_sample = config.track
    tokens = getattr(cur_sampled, track_to_sample)
    device = tokens.device

    shape = tokens.shape
    B, L = shape[0], shape[1]

    # TODO: figure out why we want this function to work with
    # _BatchedESMProteinTensor in the first place. Logics below
    # don't really work for batched tensors.
    assert B == 1

    sampling_mask = torch.ones((B, L), dtype=torch.bool, device=device)
    sampling_mask[:, 0] = False  # BOS
    # EOS and all padding tokens.
    sampling_mask &= (
        torch.arange(L).repeat(B, 1) < (sequence_lengths - 1).unsqueeze(-1)
    ).to(device)

    is_mask = _get_masked_positions(
        track_to_sample, tokens, getattr(tokenizers, track_to_sample).mask_token_id
    )
    
    # ======
    # NOTE: added allow_remask support for DPLM2-style reparameterized decoding
    if config.allow_remask and initial_mask is not None:
        # DPLM2-style mode: allow remasking and reconsidering any position that was initially masked
        # This enables the model to "reconsider" positions it already decided on
        if not initial_mask.any().item():
            raise ValueError(f"Cannot sample {config.track} when input has no initial masks.")
        sampling_mask = sampling_mask & initial_mask
    else:
        # Standard ESM3 mode: only sample positions currently masked
        if not is_mask.any().item():
            raise ValueError(f"Cannot sample {config.track} when input has no masks.")
        sampling_mask = sampling_mask & is_mask
    # ======

    # Initialize schedule and masks
    decoding_schedule = NOISE_SCHEDULE_REGISTRY[config.schedule]

    # Calculate number of tokens to sample
    still_masked = torch.sum(sampling_mask).int()
    perc_masked_after_this_step = decoding_schedule(
        torch.tensor((step + 1) / config.num_steps)
    )
    num_tokens_masked_after_this_step = (
        # To avoid rounding errors, add a small epsilon.
        # NOTE: Tensor.round does not cast to int,
        # so it actually leads to rounding down.
        # e.g. tensor(67.0000).int() = 66
        perc_masked_after_this_step * total_to_sample + 0.1
    ).int()
    num_to_sample = still_masked - num_tokens_masked_after_this_step

    if config.strategy == "entropy":
        track_entropy: torch.Tensor = getattr(entropy, track_to_sample).to(
            device
        )  # (B, L) or (B, L, D)

        if track_to_sample == "function":
            track_entropy = track_entropy.sum(-1)  # (B, L, D) -> (B, L)

        track_entropy = track_entropy.masked_fill(
            ~sampling_mask, torch.finfo(track_entropy.dtype).max
        )
        _, indices = track_entropy.topk(num_to_sample, dim=-1, largest=False)
        is_top_k = torch.zeros((B, L), dtype=torch.bool, device=device).scatter(
            1, indices, True
        )
        where_to_sample = sampling_mask & is_top_k
    # ======
    # NOTE: added stochastic sampling strategy with gumbel noise, which is similar to DPLM2's "stochastic1.0" strategy
    elif config.strategy == "stochastic":
        track_entropy: torch.Tensor = getattr(entropy, track_to_sample).to(
            device
        )  # (B, L) or (B, L, D)

        if track_to_sample == "function":
            track_entropy = track_entropy.sum(-1)  # (B, L, D) -> (B, L)
        
        gumbel_noise = -torch.log(
            -torch.log(torch.rand_like(track_entropy) + 1e-8) + 1e-8
        )
        noised_track_entropy = track_entropy + gumbel_noise * perc_masked_after_this_step
        noised_track_entropy = noised_track_entropy.masked_fill(
            ~sampling_mask, torch.finfo(noised_track_entropy.dtype).max
        )
        _, indices = noised_track_entropy.topk(num_to_sample, dim=-1, largest=False)
        is_top_k = torch.zeros((B, L), dtype=torch.bool, device=device).scatter(
            1, indices, True
        )
        where_to_sample = sampling_mask & is_top_k
    # ======
    elif config.strategy == "random":
        # Skip B since we know there is only 1 prompt here.
        _, masked_indices = sampling_mask.nonzero(as_tuple=True)
        # Random shuffle the masked indices then select the first num_to_sample.
        rnd_indices = masked_indices[torch.randperm(len(masked_indices))][
            :num_to_sample
        ]
        rnd_mask = torch.zeros_like(sampling_mask)
        rnd_mask[:, rnd_indices] = True
        where_to_sample = sampling_mask & rnd_mask

    if track_to_sample == "function":
        where_to_sample = where_to_sample.unsqueeze(-1).expand(
            B, L, tokenizers.function.depth
        )  # (B, L) -> (B, L, D)

    return where_to_sample


def _get_non_special_tokens(
    protein: ESMProteinTensor, tokenizers: TokenizerCollectionProtocol
) -> int:
    if protein.sequence is None:
        # There is no sequence to infer the number of tokens to decode.
        # So we assume the entire sequence minus bos and eos are for decoding.
        return len(protein) - 2

    mask = torch.ones_like(protein.sequence)
    for special_token in tokenizers.sequence.special_token_ids:
        if special_token == tokenizers.sequence.mask_token_id:
            continue  # MASK tokens need to be sampled.
        mask[protein.sequence == special_token] = 0

    return int(torch.sum(mask).item())

# step_ratio in [0, 1]
# annealing from initial_temperature to (initial_temperature - 1)^2 or 0.001^2
def _get_annealed_temperature(step: int, num_steps: int, initial_temperature: float):
    step_ratio = step / max(1, (num_steps - 1))
    return max(initial_temperature - step_ratio, 0.001) ** 2


def iterative_sampling_tokens(
    client: ESM3InferenceClient,
    input_tokens: list[ESMProteinTensor],
    configs: list[GenerationConfig],
    tokenizers: TokenizerCollectionProtocol,
) -> Sequence[ESMProteinTensor | ESMProteinError]:
    devices = set([t.device for t in input_tokens])
    if len(devices) > 1:
        raise AttributeError(f"Input tokens on multiple devices {devices}")

    sampled_tokens = [attr.evolve(tokens) for tokens in input_tokens]

    # Clear structure tokens if user would like to condition only on coordinates.
    for tokens, config in zip(sampled_tokens, configs):
        if config.condition_on_coordinates_only and tokens.coordinates is not None:
            tokens.structure = None

    # Total sequence lengths.
    sequence_lengths = [len(tokens) for tokens in sampled_tokens]
    # Figure out the number of tokens to be sampled for each prompt.
    total_to_sample = []
    for protein, config in zip(sampled_tokens, configs):
        track = config.track

        if getattr(protein, track) is None:
            # We need to sample the entire track.
            num_sampling_steps = _get_non_special_tokens(protein, tokenizers)
        else:
            masked = _get_masked_positions(
                track, getattr(protein, track), getattr(tokenizers, track).mask_token_id
            )
            num_sampling_steps = torch.sum(masked).item()

        total_to_sample.append(num_sampling_steps)

        # Users might over-specify the number of sampling steps for a given prompt
        # TODO: Give a warning about mismatched num_steps and number of masks.
        if (num_sampling_steps > 0) and (num_sampling_steps < config.num_steps):
            config.num_steps = int(num_sampling_steps)

    # Different prompts may ask for different number of decoding steps.
    # For now, we simply run the max number of steps.
    # TODO: return completed proteins as soon as they are finished sampling.
    max_num_steps = max([config.num_steps for config in configs])

    # Now stack the list to make a single batched ESMProteinTensor.
    # NOTE: This fills in None tracks with mask tokens
    batched_tokens = _stack_protein_tensors(
        sampled_tokens, sequence_lengths, tokenizers, devices.pop()
    )

    # ======
    # NOTE: added tracking of initial masks for allow_remask support
    # This enables DPLM2-style reparameterized decoding where positions can be reconsidered
    # IMPORTANT: Compute initial_masks AFTER batched_tokens is created, 
    # because _stack_protein_tensors fills None tracks with mask tokens
    initial_masks = []
    for i, config in enumerate(configs):
        track = config.track
        # Use batched_tokens which has masks filled in for None tracks
        track_data = getattr(batched_tokens, track)
        if config.allow_remask and track_data is not None:
            # Get the i-th prompt's track data
            prompt_track_data = track_data[i:i+1]  # Keep batch dimension
            # Track which positions were initially masked for potential remasking
            initial_mask = _get_masked_positions(
                track, prompt_track_data, getattr(tokenizers, track).mask_token_id
            )
            initial_masks.append(initial_mask)
        else:
            # No remask needed, store None
            initial_masks.append(None)
    # ======

    # Remember sampled prompts that has somehow errored out.
    errors: dict[int, ESMProteinError] = {}

    # Decode
    disable_tqdm = bool(os.environ.get("DISABLE_ITERATIVE_SAMPLING_TQDM", False))
    for t in tqdm(range(max_num_steps), disable=disable_tqdm):
        # ======
        # NOTE: CFG support — batch cond+uncond in a single forward pass
        cfg_enabled = any(c.cfg_scale > 0 for c in configs)
        if cfg_enabled:
            uncond_batched_tokens = _build_cfg_uncond_batched_tokens(
                batched_tokens, configs, tokenizers, sequence_lengths,
                input_tokens=input_tokens,
            )
            combined_tokens = _cat_batched_tokens(batched_tokens, uncond_batched_tokens)
            forward_out = _batch_forward(client, combined_tokens)
        else:
            forward_out = _batch_forward(client, batched_tokens)
        B = len(configs)
        # ======

        # Sample each prompt individually, since their configuration may
        # be very different.
        # TODO: downstream utils work with batch dimsension.
        # Group by sampling configurations and sample those prompts together.
        for i, config in enumerate(configs):  # B
            if i in errors:
                # This prompts has errored out in previous steps.
                # Skip.
                continue

            if config.track in ["coordinates", "residue_annotations"]:
                errors[i] = ESMProteinError(
                    error_code=500,
                    error_msg=f"Iterative sampling {config.track} is not supported.",
                )
                continue

            if t >= config.num_steps:
                # Done sampling for this row.
                continue

            per_prompt_cur_sampled = _BatchedESMProteinTensor.from_protein_tensor(
                batched_tokens.slice(i)
            )
            per_prompt_forward_out: LogitsOutput = _slice_tensor_dataclass(
                forward_out, i, keep_dim=True
            )

            # ======
            # NOTE: apply CFG to logits before trimming
            if cfg_enabled and config.cfg_scale > 0:
                per_prompt_uncond_out: LogitsOutput = _slice_tensor_dataclass(
                    forward_out, B + i, keep_dim=True
                )
                per_prompt_forward_out = _apply_cfg_to_logits(
                    per_prompt_forward_out, per_prompt_uncond_out,
                    config.cfg_scale, config.track,
                )
            # ======

            # Trim logits to proper sequence length for this prompt.
            per_prompt_forward_out = _trim_sequence_tensor_dataclass(
                per_prompt_forward_out,
                # Note(jungong) : we can not smiply use sequence_lenths[i] here,
                # what we want is for the sequence length of the logits to match
                # that of the prompt, which may or may not be padded, depending on
                # whether the padding was done locally with the open source model
                # (where per_prompt_cur_sampled is already padded) or by
                # BatchedESM3ModelRunner (where per_prompt_cur_sampled is not padded).
                len(per_prompt_cur_sampled),
            )

            # Handle temperature annealing, since _sample_per_prompt() doesn't have
            # the concept of decoding steps.
            if config.temperature_annealing:
                temperature = _get_annealed_temperature(
                    t, config.num_steps, config.temperature
                )
            else:
                temperature = config.temperature

            track_sample_config = SamplingTrackConfig()
            track_sample_config.invalid_ids = config.invalid_ids
            track_sample_config.temperature = temperature
            track_sample_config.top_p = config.top_p
            sampling_config = SamplingConfig(**{config.track: track_sample_config})  # type: ignore

            # Sampling has to be done per-prompt, since sampling configs
            # are likely be different for different prompts.
            # ======
            # NOTE: added the client and the enable_sequence_resample, resample_ratio, resample_temperature args
            # added allow_remask parameter for DPLM2-style reparameterized decoding
            per_prompt_forward_and_sample_output = _sample_per_prompt(
                per_prompt_cur_sampled,
                per_prompt_forward_out,
                sampling_config,
                tokenizers,
                decode_sasa_tokens=False,
                client=client,
                enable_sequence_resample=config.enable_sequence_resample,
                resample_ratio=config.resample_ratio,
                resample_temperature=config.resample_temperature,
                allow_remask=config.allow_remask,
            )
            # ======

            # All positions sampled after _sample_per_prompt() above.
            # (B, L) & (B, L, D)
            per_prompt_new_sampled = per_prompt_forward_and_sample_output.protein_tensor

            # Find the positions we should sample this round.
            assert per_prompt_forward_and_sample_output.entropy is not None
            try:
                # Pass the entropy by default. prob, top_prob, ... , logprob are ignored by now.
                where_to_sample = _get_iterative_sampling_mask_for_prompt_and_step(
                    per_prompt_cur_sampled,
                    torch.tensor(sequence_lengths[i]),
                    torch.tensor(total_to_sample[i]),
                    t,
                    per_prompt_forward_and_sample_output.entropy,
                    config,
                    tokenizers,
                    initial_mask=initial_masks[i], # NOTE
                )
            except ValueError as e:
                errors[i] = ESMProteinError(error_code=500, error_msg=str(e))
                continue

            where_to_sample.to(input_tokens[0].device)

            old_track_samples = getattr(per_prompt_cur_sampled, config.track)
            new_track_samples = getattr(per_prompt_new_sampled, config.track)

            # ======
            # NOTE: added support for remasking already sampled positions when allow_remask is True
            if config.allow_remask & (initial_masks[i] is not None):
                # get the is_mask for old_track_samples
                is_mask = _get_masked_positions(
                    config.track, old_track_samples, getattr(tokenizers, config.track).mask_token_id
                )
                # if is_mask & where_to_sample, we sample from new_track_samples, else we keep old_track_samples
                new_track_samples = torch.where(
                    where_to_sample & is_mask, new_track_samples, old_track_samples
                )
                # mask all positions that were not in where_to_sample
                new_track_samples = torch.where(
                    where_to_sample, new_track_samples, getattr(tokenizers, config.track).mask_token_id
                )
                # protect special tokens (e.g. if BOS/EOS are not masked, they should never be sampled)
                new_track_samples = torch.where(
                    initial_masks[i], new_track_samples, old_track_samples
                )
            else:
                # Iterative sampling by picking the tokens sampled this round
                # from new_track_samples to old_track_samples.
                new_track_samples = torch.where(
                    where_to_sample, new_track_samples, old_track_samples
                )
            # ======

            # Update the corresponding row with new data.
            getattr(batched_tokens, config.track)[i, ...] = new_track_samples[0]

    # Un-pack to a list of single ProteinTypes.
    output_tokens = [
        batched_tokens.slice(i, sequence_len=sequence_lengths[i])
        if i not in errors
        else errors[i]
        for i in range(len(input_tokens))
    ]

    # Do not update tracks that were not sampled (e.g. keep None instead of masks)
    for inputs, outputs, config in zip(input_tokens, output_tokens, configs):
        if isinstance(outputs, ESMProteinError):
            continue

        # First restore coordinates field.
        # We know coordinates can never be iteratively sampled.
        setattr(outputs, "coordinates", getattr(inputs, "coordinates"))
        # Maybe restore all the other fields.
        for f in attr.fields(SamplingConfig):
            if "embedding" in f.name or f.name == "return_hidden_states":
                continue
            if f.name != config.track:
                setattr(outputs, f.name, getattr(inputs, f.name))

    return output_tokens


def _batch_forward(client: ESM3InferenceClient, protein: _BatchedESMProteinTensor):
    # Forward pass
    return client.logits(
        protein,
        LogitsConfig(
            sequence=True,
            structure=True,
            secondary_structure=True,
            sasa=True,
            function=True,
            residue_annotations=True,
            return_embeddings=True,
        ),
    )


# ======
# NOTE: CFG helper functions for Classifier-Free Guidance
def _build_cfg_uncond_batched_tokens(
    batched_tokens: _BatchedESMProteinTensor,
    configs: list[GenerationConfig],
    tokenizers: TokenizerCollectionProtocol,
    sequence_lengths: list[int],
    input_tokens: list[ESMProteinTensor] | None = None,
) -> _BatchedESMProteinTensor:
    """Build unconditional batched tokens for CFG by masking condition tracks.

    For 'drop_condition' mode: mask all tracks except the target track.
    For 'full_mask' mode: mask all tracks including the target.
    For 'drop_motif' mode: mask motif positions across all tracks (requires input_tokens).
    NOTE: 'cross_modal' mode is not supported here. This function works for track-wise sampling, while cross modal CFG only make sense in the multitrack "cogen" sampling.
    """
    uncond = _BatchedESMProteinTensor()
    for f in attr.fields(ESMProteinTensor):
        if f.name == "potential_sequence_of_concern":
            continue
        val = getattr(batched_tokens, f.name)
        setattr(uncond, f.name, val.clone() if val is not None else None)

    track_names = [
        f.name for f in attr.fields(ESMProteinTensor)
        if f.name != "potential_sequence_of_concern"
    ]

    for i, config in enumerate(configs):
        if config.cfg_scale <= 0:
            continue

        target_track = config.track
        cfg_mode = config.cfg_mode
        seq_len = sequence_lengths[i]

        if cfg_mode == "drop_condition":
            tracks_to_mask = [t for t in track_names if t != target_track]
        elif cfg_mode == "full_mask":
            tracks_to_mask = list(track_names)
        elif cfg_mode == "drop_motif":
            # Mask motif positions across all tracks.
            # Motif positions are identified from the original input_tokens.
            assert input_tokens is not None, "input_tokens required for drop_motif mode"
            orig = input_tokens[i]
            seq_mask_token_id = tokenizers.sequence.mask_token_id
            # Motif = non-mask positions in original sequence + finite coordinates
            motif_mask = orig.sequence != seq_mask_token_id  # [L]
            if orig.coordinates is not None:
                coord_motif = torch.isfinite(orig.coordinates).any(dim=-1).any(dim=-1)  # [L]
                motif_mask = motif_mask | coord_motif
            # Exclude BOS and EOS (must be after OR to avoid re-enabling)
            motif_mask[0] = False
            motif_mask[-1] = False
            L = len(motif_mask)
            for track_name in track_names:
                track_data = getattr(uncond, track_name)
                if track_data is None:
                    continue
                if track_name == "coordinates":
                    track_data[i, :L][motif_mask] = torch.inf
                else:
                    mask_token_id = getattr(tokenizers, track_name).mask_token_id
                    track_data[i, :L][motif_mask] = mask_token_id
            continue
        else:
            continue

        for track_name in tracks_to_mask:
            track_data = getattr(uncond, track_name)
            if track_data is None:
                continue

            if track_name == "coordinates":
                track_data[i] = torch.inf
            else:
                mask_token_id = getattr(tokenizers, track_name).mask_token_id
                # Preserve BOS and EOS tokens
                bos = track_data[i, 0].clone()
                eos = track_data[i, seq_len - 1].clone()
                track_data[i] = mask_token_id
                track_data[i, 0] = bos
                track_data[i, seq_len - 1] = eos

    return uncond
# ======

# ======
# NOTE: CFG helper functions for Classifier-Free Guidance
def _cat_batched_tokens(
    cond: _BatchedESMProteinTensor,
    uncond: _BatchedESMProteinTensor,
) -> _BatchedESMProteinTensor:
    """Concatenate conditional and unconditional batched tokens along batch dim."""
    combined = _BatchedESMProteinTensor()
    for f in attr.fields(ESMProteinTensor):
        if f.name == "potential_sequence_of_concern":
            continue
        cond_val = getattr(cond, f.name)
        uncond_val = getattr(uncond, f.name)
        if cond_val is not None and uncond_val is not None:
            setattr(combined, f.name, torch.cat([cond_val, uncond_val], dim=0))
        else:
            # Both should be None or both should be tensors after _stack_protein_tensors
            setattr(combined, f.name, None)
    return combined
# ======

# ======
# NOTE: CFG helper functions for Classifier-Free Guidance
def _apply_cfg_to_logits(
    cond_out: LogitsOutput,
    uncond_out: LogitsOutput,
    cfg_scale: float,
    target_track: str,
) -> LogitsOutput:
    """Apply CFG formula on the target track: guided = uncond + cfg_scale * (cond - uncond)."""
    if cond_out.logits is None or uncond_out.logits is None:
        return cond_out

    guided_logits_dict = {}
    for f in attr.fields(ForwardTrackData):
        cond_logit = getattr(cond_out.logits, f.name)
        uncond_logit = getattr(uncond_out.logits, f.name)
        if f.name == target_track and cond_logit is not None and uncond_logit is not None:
            guided_logits_dict[f.name] = uncond_logit + cfg_scale * (cond_logit - uncond_logit)
        else:
            guided_logits_dict[f.name] = cond_logit

    guided_logits = ForwardTrackData(**guided_logits_dict)
    return attr.evolve(cond_out, logits=guided_logits)
# ======


# ======
# NOTE: CFG helper functions for multi-track generation
def _build_cfg_uncond_batched_tokens_multitrack(
    batched_tokens: _BatchedESMProteinTensor,
    configs: list[MultiTrackGenerationConfig],
    tokenizers: TokenizerCollectionProtocol,
    sequence_lengths: list[int],
    input_tokens: list[ESMProteinTensor] | None = None,
    cfg_mode: str = "drop_motif",
) -> _BatchedESMProteinTensor | tuple[_BatchedESMProteinTensor, _BatchedESMProteinTensor]:
    """Build unconditional batched tokens for multi-track CFG.

    For 'drop_motif': mask motif positions across all tracks → single uncond.
    For 'drop_condition'/'full_mask': mask condition tracks → single uncond.
    For 'cross_modal': build two unconds (mask-seq, mask-struct) → tuple of two.
    For 'motif_anchored_cross_modal': like cross_modal but only mask scaffold (non-motif) positions.
    For 'drop_motif_cross_modal': mask motif on own track + mask all on cross track.
    """
    track_names = [
        f.name for f in attr.fields(ESMProteinTensor)
        if f.name != "potential_sequence_of_concern"
    ]

    def _clone_batch():
        uncond = _BatchedESMProteinTensor()
        for f in attr.fields(ESMProteinTensor):
            if f.name == "potential_sequence_of_concern":
                continue
            val = getattr(batched_tokens, f.name)
            setattr(uncond, f.name, val.clone() if val is not None else None)
        return uncond

    if cfg_mode == "cross_modal":
        # uncond_for_seq: mask structure/coordinates tracks (keep sequence)
        # uncond_for_struct: mask sequence track (keep structure/coordinates)
        uncond_for_seq = _clone_batch()
        uncond_for_struct = _clone_batch()

        struct_tracks = ["structure", "coordinates", "secondary_structure", "sasa"]
        seq_tracks = ["sequence"]

        for i, config in enumerate(configs):
            seq_len = sequence_lengths[i]
            # uncond_for_seq: mask struct-related tracks
            for track_name in struct_tracks:
                track_data = getattr(uncond_for_seq, track_name)
                if track_data is None:
                    continue
                if track_name == "coordinates":
                    track_data[i] = torch.inf
                else:
                    mask_token_id = getattr(tokenizers, track_name).mask_token_id
                    bos = track_data[i, 0].clone()
                    eos = track_data[i, seq_len - 1].clone()
                    track_data[i] = mask_token_id
                    track_data[i, 0] = bos
                    track_data[i, seq_len - 1] = eos

            # uncond_for_struct: mask sequence track
            for track_name in seq_tracks:
                track_data = getattr(uncond_for_struct, track_name)
                if track_data is None:
                    continue
                mask_token_id = getattr(tokenizers, track_name).mask_token_id
                bos = track_data[i, 0].clone()
                eos = track_data[i, seq_len - 1].clone()
                track_data[i] = mask_token_id
                track_data[i, 0] = bos
                track_data[i, seq_len - 1] = eos

        return uncond_for_seq, uncond_for_struct

    if cfg_mode == "motif_anchored_cross_modal":
        # Like cross_modal, but only mask scaffold (non-motif) positions of the other track.
        # Motif positions are preserved → guidance targets scaffold cross-modal consistency.
        assert input_tokens is not None, "input_tokens required for motif_anchored_cross_modal mode"
        uncond_for_seq = _clone_batch()
        uncond_for_struct = _clone_batch()

        struct_tracks = ["structure", "coordinates", "secondary_structure", "sasa"]
        seq_tracks = ["sequence"]

        for i, config in enumerate(configs):
            seq_len = sequence_lengths[i]
            orig = input_tokens[i]

            # Build motif mask: True for motif positions (positions with known seq or coords)
            seq_mask_token_id = tokenizers.sequence.mask_token_id
            motif_mask = orig.sequence != seq_mask_token_id
            if orig.coordinates is not None:
                coord_motif = torch.isfinite(orig.coordinates).any(dim=-1).any(dim=-1)
                motif_mask = motif_mask | coord_motif
            motif_mask[0] = False   # BOS
            motif_mask[-1] = False  # EOS
            L = len(motif_mask)
            scaffold_mask = ~motif_mask  # True for scaffold positions

            # uncond_for_seq: mask scaffold struct positions (keep motif struct + all seq)
            for track_name in struct_tracks:
                track_data = getattr(uncond_for_seq, track_name)
                if track_data is None:
                    continue
                if track_name == "coordinates":
                    track_data[i, :L][scaffold_mask] = torch.inf
                else:
                    mask_token_id = getattr(tokenizers, track_name).mask_token_id
                    track_data[i, :L][scaffold_mask] = mask_token_id

            # uncond_for_struct: mask scaffold seq positions (keep motif seq + all struct)
            for track_name in seq_tracks:
                track_data = getattr(uncond_for_struct, track_name)
                if track_data is None:
                    continue
                mask_token_id = getattr(tokenizers, track_name).mask_token_id
                track_data[i, :L][scaffold_mask] = mask_token_id

        return uncond_for_seq, uncond_for_struct

    if cfg_mode == "drop_motif_cross_modal":
        # Combine drop_motif + cross_modal:
        # uncond_for_seq: mask motif seq positions + mask ALL struct tracks
        # uncond_for_struct: mask motif struct positions/coords + mask ALL seq track
        # Guidance direction: motif faithfulness + full cross-modal consistency
        assert input_tokens is not None, "input_tokens required for drop_motif_cross_modal mode"
        uncond_for_seq = _clone_batch()
        uncond_for_struct = _clone_batch()

        struct_tracks = ["structure", "coordinates", "secondary_structure", "sasa"]
        seq_tracks = ["sequence"]

        for i, config in enumerate(configs):
            seq_len = sequence_lengths[i]
            orig = input_tokens[i]

            # Build motif mask
            seq_mask_token_id = tokenizers.sequence.mask_token_id
            motif_mask = orig.sequence != seq_mask_token_id
            if orig.coordinates is not None:
                coord_motif = torch.isfinite(orig.coordinates).any(dim=-1).any(dim=-1)
                motif_mask = motif_mask | coord_motif
            motif_mask[0] = False   # BOS
            motif_mask[-1] = False  # EOS
            L = len(motif_mask)

            # uncond_for_seq: mask motif seq positions + mask ALL struct tracks
            seq_data = getattr(uncond_for_seq, "sequence")
            if seq_data is not None:
                mask_token_id = tokenizers.sequence.mask_token_id
                seq_data[i, :L][motif_mask] = mask_token_id
            for track_name in struct_tracks:
                track_data = getattr(uncond_for_seq, track_name)
                if track_data is None:
                    continue
                if track_name == "coordinates":
                    track_data[i] = torch.inf
                else:
                    mask_token_id = getattr(tokenizers, track_name).mask_token_id
                    bos = track_data[i, 0].clone()
                    eos = track_data[i, seq_len - 1].clone()
                    track_data[i] = mask_token_id
                    track_data[i, 0] = bos
                    track_data[i, seq_len - 1] = eos

            # uncond_for_struct: mask motif struct positions/coords + mask ALL seq track
            for track_name in struct_tracks:
                track_data = getattr(uncond_for_struct, track_name)
                if track_data is None:
                    continue
                if track_name == "coordinates":
                    track_data[i, :L][motif_mask] = torch.inf
                else:
                    mask_token_id = getattr(tokenizers, track_name).mask_token_id
                    track_data[i, :L][motif_mask] = mask_token_id
            seq_data = getattr(uncond_for_struct, "sequence")
            if seq_data is not None:
                mask_token_id = tokenizers.sequence.mask_token_id
                bos = seq_data[i, 0].clone()
                eos = seq_data[i, seq_len - 1].clone()
                seq_data[i] = mask_token_id
                seq_data[i, 0] = bos
                seq_data[i, seq_len - 1] = eos

        return uncond_for_seq, uncond_for_struct

    # Non-cross_modal modes: single uncond
    uncond = _clone_batch()

    for i, config in enumerate(configs):
        seq_len = sequence_lengths[i]

        if cfg_mode == "drop_motif":
            assert input_tokens is not None, "input_tokens required for drop_motif mode"
            orig = input_tokens[i]
            seq_mask_token_id = tokenizers.sequence.mask_token_id
            motif_mask = orig.sequence != seq_mask_token_id
            if orig.coordinates is not None:
                coord_motif = torch.isfinite(orig.coordinates).any(dim=-1).any(dim=-1)
                motif_mask = motif_mask | coord_motif
            motif_mask[0] = False
            motif_mask[-1] = False
            L = len(motif_mask)
            for track_name in track_names:
                track_data = getattr(uncond, track_name)
                if track_data is None:
                    continue
                if track_name == "coordinates":
                    track_data[i, :L][motif_mask] = torch.inf
                else:
                    mask_token_id = getattr(tokenizers, track_name).mask_token_id
                    track_data[i, :L][motif_mask] = mask_token_id

        elif cfg_mode in ("drop_condition", "full_mask"):
            # Mask all tracks for simplicity in multitrack
            for track_name in track_names:
                track_data = getattr(uncond, track_name)
                if track_data is None:
                    continue
                if track_name == "coordinates":
                    track_data[i] = torch.inf
                else:
                    mask_token_id = getattr(tokenizers, track_name).mask_token_id
                    bos = track_data[i, 0].clone()
                    eos = track_data[i, seq_len - 1].clone()
                    track_data[i] = mask_token_id
                    track_data[i, 0] = bos
                    track_data[i, seq_len - 1] = eos

    return uncond
# ======

# ======
# NOTE: CFG helper functions for multi-track generation
def _apply_cfg_to_logits_multitrack(
    cond_out: LogitsOutput,
    uncond_out: LogitsOutput,
    track_cfg_scales: dict[str, float],
) -> LogitsOutput:
    """Apply per-track CFG for multi-track generation."""
    if cond_out.logits is None or uncond_out.logits is None:
        return cond_out

    guided_logits_dict = {}
    for f in attr.fields(ForwardTrackData):
        cond_logit = getattr(cond_out.logits, f.name)
        uncond_logit = getattr(uncond_out.logits, f.name)
        cfg_scale = track_cfg_scales.get(f.name, 0.0)
        if cfg_scale > 0 and cond_logit is not None and uncond_logit is not None:
            guided_logits_dict[f.name] = uncond_logit + cfg_scale * (cond_logit - uncond_logit)
        else:
            guided_logits_dict[f.name] = cond_logit

    guided_logits = ForwardTrackData(**guided_logits_dict)
    return attr.evolve(cond_out, logits=guided_logits)
# ======

# ======
# NOTE: CFG helper functions for multi-track generation (cross_modal version)
def _apply_cfg_to_logits_cross_modal(
    cond_out: LogitsOutput,
    uncond_seq_out: LogitsOutput,
    uncond_struct_out: LogitsOutput,
    track_cfg_scales: dict[str, float],
) -> LogitsOutput:
    """Apply cross-modal CFG: each track uses the uncond where its cross-modal info was masked."""
    if cond_out.logits is None:
        return cond_out

    # sequence logits guided by uncond_for_seq (struct was masked → boosts struct→seq influence)
    # structure logits guided by uncond_for_struct (seq was masked → boosts seq→struct influence)
    seq_uncond_mapping = {
        "sequence": uncond_seq_out,    # struct masked
        "structure": uncond_struct_out,  # seq masked
        "secondary_structure": uncond_struct_out,
        "sasa": uncond_struct_out,
        "function": uncond_seq_out,
    }

    guided_logits_dict = {}
    for f in attr.fields(ForwardTrackData):
        cond_logit = getattr(cond_out.logits, f.name)
        cfg_scale = track_cfg_scales.get(f.name, 0.0)
        uncond_for_track = seq_uncond_mapping.get(f.name)
        if cfg_scale > 0 and cond_logit is not None and uncond_for_track is not None:
            uncond_logit = getattr(uncond_for_track.logits, f.name)
            if uncond_logit is not None:
                guided_logits_dict[f.name] = uncond_logit + cfg_scale * (cond_logit - uncond_logit)
            else:
                guided_logits_dict[f.name] = cond_logit
        else:
            guided_logits_dict[f.name] = cond_logit

    guided_logits = ForwardTrackData(**guided_logits_dict)
    return attr.evolve(cond_out, logits=guided_logits)
# ======


# ======
# NOTE: new function for sequence resampling
# details refer to the resample section in DPLM: dplm/src/byprot/models/dplm/dplm.py
def _check_and_resample_sequence(
    client: ESM3InferenceClient,
    protein: _BatchedESMProteinTensor,
    sampled_tokens: torch.Tensor,
    ratio: float = 0.25,
    top_p: float = 0.95,
    temperature: float = 1.0,
    mask_idx: int = 32,
    invalid_ids: list[int] = [],
) -> torch.Tensor:
    """
    Rejection sampling for eliminating unexpected repeat patterns in sequence generation.
    Adapted from DPLM's resample mechanism.
    
    Args:
        client: ESM3 inference client for model forward pass
        protein: Current protein tensor state
        sampled_tokens: Newly sampled sequence tokens [B, L]
        ratio: Threshold ratio for detecting high-frequency tokens (default: 0.25)
        top_p: Top-p filtering parameter for resampling (default: 0.95)
        temperature: Temperature for resampling (default: 1.0)
        mask_idx: Mask token index
        invalid_ids: Invalid token IDs to exclude from sampling
        
    Returns:
        Corrected sequence tokens [B, L]
    """
    batch_size, seq_len = sampled_tokens.shape
    to_be_resample_idx = []
    resample_input_list = []
    resample_input_mask_list = []
    
    # Calculate the frequency of all tokens for each sequence
    for i in range(batch_size):
        seq = sampled_tokens[i]
        token_dict = {}
        max_token_count = 0
        
        # Count token frequencies (excluding special tokens at position 0 and -1)
        for j in range(1, seq_len - 1):
            token = int(seq[j].item())
            if token not in token_dict:
                token_dict[token] = []
            token_dict[token].append(j)
            max_token_count = max(max_token_count, len(token_dict[token]))
        
        # Check if any token exceeds the threshold
        if max_token_count > (seq_len - 2) * ratio:  # -2 for BOS and EOS
            to_be_resample_idx.append(i)
            
            # Create mask for high-frequency tokens
            mask = torch.zeros(seq_len, dtype=torch.bool, device=seq.device)
            for token, positions in token_dict.items():
                if len(positions) > (seq_len - 2) * ratio:
                    for pos in positions:
                        mask[pos] = True
            
            resample_input_mask_list.append(mask)
            # Mask out high-frequency tokens
            masked_seq = seq.clone()
            masked_seq[mask] = mask_idx
            resample_input_list.append(masked_seq)
    
    # If no sequences need resampling, return original tokens
    if len(to_be_resample_idx) == 0:
        return sampled_tokens
    
    # Prepare resampling batch
    resample_protein = attr.evolve(protein)
    resample_indices = torch.tensor(to_be_resample_idx, device=sampled_tokens.device)
    
    # Update sequence tokens for resampling
    for idx, seq_idx in enumerate(to_be_resample_idx):
        resample_protein.sequence[seq_idx] = resample_input_list[idx]
    
    # Forward pass to get new logits
    resample_logits_output = client.logits(
        resample_protein, 
        LogitsConfig(sequence=True, return_embeddings=False)
    )
    
    if resample_logits_output.logits is None or resample_logits_output.logits.sequence is None:
        # If logits unavailable, return original tokens
        return sampled_tokens
    
    resample_logits = resample_logits_output.logits.sequence[resample_indices]
    
    # Apply top-p filtering
    if top_p < 1.0:
        resample_logits = top_p_logits(resample_logits, top_p=top_p)
    
    # Mask out invalid token IDs
    for invalid_id in invalid_ids:
        resample_logits[..., invalid_id] = float('-inf')
    
    # Sample with temperature
    if temperature == 0:
        resampled_tokens = resample_logits.argmax(dim=-1)
    else:
        probs = torch.softmax(resample_logits / temperature, dim=-1)
        resampled_tokens = torch.multinomial(
            probs.view(-1, probs.shape[-1]), 
            num_samples=1
        ).view(probs.shape[:-1])
    
    # Update only the masked positions
    result_tokens = sampled_tokens.clone()
    for idx, seq_idx in enumerate(to_be_resample_idx):
        mask = resample_input_mask_list[idx]
        result_tokens[seq_idx][mask] = resampled_tokens[idx][mask]
    
    return result_tokens
# ======

# ======
# NOTE: updated _sample_per_prompt to support sequence resampling and allow_remask
# added the client and the enable_sequence_resample, resample_ratio, resample_temperature args
# added allow_remask parameter to enable DPLM2-style reparameterized decoding
def _sample_per_prompt(
    protein: _BatchedESMProteinTensor,
    logits_output: LogitsOutput,
    sampling_config: SamplingConfig,
    tokenizers: TokenizerCollectionProtocol,
    decode_sasa_tokens: bool = True,
    mask_logits_of_invalid_ids: bool = True,
    client: ESM3InferenceClient | None = None,
    enable_sequence_resample: bool = False,
    resample_ratio: float = 0.25,
    resample_temperature: float = None,
    allow_remask: bool = False,
) -> ForwardAndSampleOutput:
# ======
    assert logits_output.logits is not None

    def maybe_clone(x: torch.Tensor | None) -> torch.Tensor | None:
        return x.clone() if x is not None else None

    # Sampling
    tokens_dir = {}
    track_sampling_metadata_dir: dict[str, dict | None] = {}
    integer_sampling_tracks = ["sequence", "structure", "secondary_structure"]
    if not decode_sasa_tokens:
        integer_sampling_tracks.append("sasa")

    for track in integer_sampling_tracks:
        config = getattr(sampling_config, track)
        if config is None:
            tokens_dir[track] = maybe_clone(getattr(protein, track))
            continue
        tokenizer = getattr(tokenizers, track)
        valid_ids = (
            set(tokenizer.all_token_ids)
            - set(tokenizer.special_token_ids)
            - set(config.invalid_ids)
        )
        
        # ======
        # NOTE: when allow_remask=True, disable only_sample_masked_tokens
        # to allow remasking and reconsidering already-decoded positions (DPLM2-style reparameterized decoding)
        sampling_track_config = config
        if allow_remask:
            # Create a copy with only_sample_masked_tokens disabled
            sampling_track_config = attr.evolve(config, only_sample_masked_tokens=False)
        # ======
        
        # Including the 'entropy', 'prob', 'logprob', 'top_prob', 'topk_logprob', and 'topk_tokens'
        sampling_metadata = _sample_track(
            logits=getattr(logits_output.logits, track),
            tokens=getattr(protein, track),
            sampling_track_config=sampling_track_config,
            mask_idx=getattr(tokenizers, track).mask_token_id,
            valid_ids=list(valid_ids),
            mask_logits_of_invalid_ids=mask_logits_of_invalid_ids,
        )
        # ======
        # NOTE: added sequence resampling logic
        sampled_tokens = sampling_metadata.pop("sampled_tokens")  # (L,)
        # Apply resample for sequence track to avoid pathological repetitions
        if track == "sequence" and enable_sequence_resample and client is not None:
            sampled_tokens = _check_and_resample_sequence(
                client=client,
                protein=protein,
                sampled_tokens=sampled_tokens,
                ratio=resample_ratio,
                top_p=config.top_p,
                temperature=resample_temperature if resample_temperature is not None else config.temperature,
                mask_idx=getattr(tokenizers, track).mask_token_id,
                invalid_ids=config.invalid_ids,
            )
        tokens_dir[track] = sampled_tokens
        # ======
        track_sampling_metadata_dir[track] = sampling_metadata

    # Sample SASA seperately (if needed)
    if decode_sasa_tokens:
        config = getattr(sampling_config, "sasa")
        track_sampling_metadata_dir["sasa"] = None

        if config is None:
            tokens_dir["sasa"] = maybe_clone(getattr(protein, "sasa"))
        else:
            if config.topk_logprobs > 0:
                warn("For SASA sampling, 'topk_logprobs' is expected to be 0.")

            assert logits_output.logits.sasa is not None
            assert protein.sasa is not None

            valid_ids = (
                set(tokenizers.sasa.all_token_ids)
                - set(tokenizers.sasa.special_token_ids)
                - set(config.invalid_ids)
            )
            sasa_logits = logits_output.logits.sasa
            sasa_value = sample_sasa_logits(
                sasa_logits,
                protein.sasa,
                sampling_track_config=config,
                mask_idx=tokenizers.sasa.mask_token_id,
                valid_ids=list(valid_ids),
                mask_logits_of_invalid_ids=mask_logits_of_invalid_ids,
            )
            tokens_dir["sasa"] = sasa_value

            probs = sasa_logits.softmax(dim=-1)
            # Note(tjia): sasa_logits can have -inf because of invalid ids, so
            # probs * sasa_logits.log_softmax(-1) is nan. We need to set
            # those positions to 0 to get the correct entropy value
            entropy = -(torch.nan_to_num(probs * sasa_logits.log_softmax(-1))).sum(-1)

            track_sampling_metadata_dir["sasa"] = {"entropy": entropy}

    # Sample function and residue annotations separately
    config = getattr(sampling_config, "function")
    function_logits = getattr(logits_output.logits, "function")
    if config is None or function_logits is None:
        tokens_dir["function"] = maybe_clone(getattr(protein, "function"))
        tokens_dir["residue_annotations"] = maybe_clone(
            getattr(protein, "residue_annotations")
        )
    else:
        if config.invalid_ids is not None and len(config.invalid_ids) > 0:
            warn("For function sampling, invalid_ids sampling config is not supported.")

        sampling_metadata = _sample_function_track(
            tokenizers.function,
            tokens=getattr(protein, "function"),
            logits=function_logits,
            sampling_track_config=config,
        )
        tokens_dir["function"] = sampling_metadata.pop("sampled_tokens")  # (L, D)
        track_sampling_metadata_dir["function"] = sampling_metadata

        sampled_tokens, _ = sample_residue_annotation_logits(
            logits=logits_output.residue_annotation_logits  # type: ignore
        )
        tokens_dir["residue_annotations"] = sampled_tokens  # (L, MAX_R)

    # Format output
    forward_and_sample_output_dir = {}
    forward_and_sample_output_dir["protein_tensor"] = ESMProteinTensor(**tokens_dir)
    for property in [
        "entropy",
        "prob",
        "logprob",
        "top_prob",
        "topk_logprob",
        "topk_tokens",
    ]:
        is_all_none = True
        forward_track_data_dir = {}
        for track in track_sampling_metadata_dir.keys():
            values = track_sampling_metadata_dir[track]
            if values is not None and values.get(property, None) is not None:
                forward_track_data_dir[track] = values.get(property, None)
                is_all_none = False
        if not is_all_none:
            forward_and_sample_output_dir[property] = ForwardTrackData(
                **forward_track_data_dir
            )
        else:
            forward_and_sample_output_dir[property] = None

    per_res_embed = (
        logits_output.embeddings  # type: ignore
        if sampling_config.return_per_residue_embeddings
        else None
    )
    mean_embedding = (
        # [B, L, D] -> [B, D]
        logits_output.embeddings.mean(dim=1)  # type: ignore
        if sampling_config.return_mean_embedding
        else None
    )

    # Including the 'protein_tensor' (sequence, structure, secondary_structure...), 'entropy', 'prob', 'logprob', 'top_prob', 'topk_logprob', and 'topk_tokens'
    return ForwardAndSampleOutput(
        per_residue_embedding=per_res_embed,
        mean_embedding=mean_embedding,
        **forward_and_sample_output_dir,
    )


def _sample_track(
    logits: torch.Tensor,
    tokens: torch.Tensor,
    sampling_track_config: SamplingTrackConfig,
    mask_idx: int,
    valid_ids: list[int],
    mask_logits_of_invalid_ids: bool = True,
) -> dict[str, torch.Tensor]:
    """Works with inputs that have batch dimension."""
    # Sample in all positions
    temperature = sampling_track_config.temperature
    # We have to trim the logits and sampled tokens at potentially padded slots
    # since the logits may be computed with a longer padded batch, while tokens
    # are the original input sequence.
    sampled_tokens = sample_logits(
        logits,
        temperature=temperature,
        valid_ids=valid_ids,
        top_p=sampling_track_config.top_p,
        mask_logits_of_invalid_ids=mask_logits_of_invalid_ids,
    )
    log_probs = logits.log_softmax(-1)
    sampling_mask = get_sampling_mask(tokens, sampling_track_config, mask_idx)
    sampled_tokens = torch.where(sampling_mask, sampled_tokens, tokens)

    return _compute_track_metadata(
        sampled_tokens,
        log_probs,
        sampling_mask,
        top_k=sampling_track_config.topk_logprobs,
    )


def _sample_function_track(
    function_tokenizer: InterProQuantizedTokenizer,
    tokens: torch.Tensor,
    logits: torch.Tensor,
    sampling_track_config: SamplingTrackConfig,
) -> dict[str, torch.Tensor]:
    """Works with inputs that have batch dimension."""
    # Do not sample at BOS and EOS tokens
    sampling_mask = torch.ones_like(tokens, dtype=torch.bool)[..., 0]  # (B, L)
    sampling_mask[..., 0] = False
    sampling_mask[..., -1] = False

    sampled_tokens, logprobs = sample_function_logits(
        logits,
        function_tokenizer,
        top_p=sampling_track_config.top_p,
        temperature=sampling_track_config.temperature,
    )
    if sampling_track_config.only_sample_masked_tokens:
        is_mask = torch.all(
            tokens == function_tokenizer.mask_token_id, dim=-1
        )  # (B, L)
        sampling_mask = sampling_mask & is_mask

    sampled_tokens = torch.where(
        sampling_mask[..., None].expand_as(sampled_tokens), sampled_tokens, tokens
    )  # (B, L, D)

    # Set logprobs for non-sampled tokens to 0
    logprobs_null = torch.full_like(logprobs, -torch.inf)  # (B, L, D, V)
    logprobs_null = torch.scatter(
        logprobs_null, -1, tokens[..., None], torch.zeros_like(logprobs_null)[..., [0]]
    )
    logprobs = torch.where(
        sampling_mask[..., None, None].expand_as(logprobs), logprobs, logprobs_null
    )  # (B, L, D, V)

    function_metadata = _compute_track_metadata(
        sampled_tokens,
        logprobs,
        sampling_mask,
        top_k=sampling_track_config.topk_logprobs,
    )
    # Consider the entropy of the joint distribution of all function tokens at each position
    function_metadata["entropy"] = function_metadata["entropy"].sum(
        -1
    )  # (B, L, D) -> (B, L)
    return function_metadata


def _compute_track_metadata(
    sampled_tokens: torch.Tensor,
    log_probs: torch.Tensor,
    sampling_mask: torch.Tensor,
    top_k: int,
) -> dict:
    """Works with inputs that have batch dimension."""
    probs = torch.exp(log_probs)  # (B, L)
    entropy = torch.distributions.Categorical(logits=log_probs).entropy()  # (B, L)

    # Only compute probabilities for sampled tokens
    sampled_logprob = torch.zeros_like(sampled_tokens, dtype=log_probs.dtype)  # (B, L)

    if sampled_tokens.dim() > sampling_mask.dim():
        assert sampled_tokens.dim() == 3  # (B, L, D)
        assert sampling_mask.dim() == 2  # (B, L)
        sampling_mask = sampling_mask[..., None].expand_as(sampled_tokens)

    sampled_tokens_valid = sampled_tokens[sampling_mask]
    sampled_log_probs_valid = log_probs[sampling_mask, sampled_tokens_valid]
    sampled_logprob[sampling_mask] = sampled_log_probs_valid

    # Calculate extra metadata
    sampled_prob = torch.exp(sampled_logprob)
    top_prob = torch.max(probs, dim=-1).values
    topk_logprobs, topk_tokens = torch.topk(log_probs, top_k, dim=-1)
    topk_logprobs = None if top_k == 0 else topk_logprobs
    topk_tokens = None if top_k == 0 else topk_tokens

    return {
        "entropy": entropy,
        "sampled_tokens": sampled_tokens,
        "prob": sampled_prob,
        "logprob": sampled_logprob,
        "top_prob": top_prob,
        "topk_logprob": topk_logprobs,
        "topk_tokens": topk_tokens,
    }

# ======
# NOTE: Multi-track iterative sampling
def _get_iterative_sampling_masks_for_multitrack(
    cur_sampled: _BatchedESMProteinTensor,
    sequence_lengths: torch.Tensor,
    total_to_sample: dict[str, torch.Tensor],
    step: int,
    entropy: ForwardTrackData,
    multitrack_config: MultiTrackGenerationConfig,
    tokenizers: TokenizerCollectionProtocol,
    initial_masks: dict[str, torch.Tensor] | None = None, # NOTE
) -> dict[str, torch.Tensor]:
    """Get sampling masks for all tracks based on forward output and multitrack config.
    
    Args:
        cur_sampled: Current sampled protein tensor
        sequence_lengths: Sequence lengths for each batch
        total_to_sample: Dict mapping track names to total tokens to sample for each track
        step: Current step in iterative sampling
        entropy: Entropy values from forward pass
        multitrack_config: MultiTrackGenerationConfig with per-track parameters
        tokenizers: Tokenizer collection
        initial_masks: Dict mapping track names to initial mask tensors. 
                      If provided and track's allow_remask=True, allows remasking and reconsidering any
                      position that was initially masked (DPLM2-style reparameterized decoding).
        
    Returns:
        Dict mapping track names to sampling masks
    """
    B, L = cur_sampled.sequence.shape[0], cur_sampled.sequence.shape[1]
    assert B == 1, "Multitrack sampling currently only supports batch size 1"
    
    device = cur_sampled.sequence.device
    where_to_sample_dict = {}
    
    for track_name, track_config in multitrack_config.tracks.items():
        # Get tokens and tokenizer for this track
        tokens = getattr(cur_sampled, track_name)
        if tokens is None:
            continue
            
        tokenizer = getattr(tokenizers, track_name)
        
        # Create base sampling mask (excluding BOS, EOS, padding)
        sampling_mask = torch.ones((B, L), dtype=torch.bool, device=device)
        sampling_mask[:, 0] = False  # BOS
        sampling_mask &= (
            torch.arange(L).repeat(B, 1) < (sequence_lengths - 1).unsqueeze(-1)
        ).to(device)
        
        # Get masked positions
        is_mask = _get_masked_positions(
            track_name, tokens, tokenizer.mask_token_id
        )
        
        # ======
        # NOTE: added allow_remask support for DPLM2-style reparameterized decoding
        if track_config.allow_remask and initial_masks is not None and track_name in initial_masks:
            # DPLM2-style mode: allow remasking and reconsidering any position that was initially masked
            sampling_mask = sampling_mask & initial_masks[track_name]
        else:
            # Standard ESM3 mode: only sample positions currently masked
            if not is_mask.any().item():
                # No masked positions for this track, skip
                continue
            sampling_mask = sampling_mask & is_mask
        # ======
        
        # Get noise schedule for this track
        decoding_schedule = NOISE_SCHEDULE_REGISTRY[track_config.schedule]
        
        # Calculate number of tokens to sample for this track
        still_masked = torch.sum(sampling_mask).int()
        
        # Use track's own num_steps for progress calculation
        perc_masked_after_this_step = decoding_schedule(
            torch.tensor((step + 1) / track_config.num_steps)
        )
        num_tokens_masked_after_this_step = (
            perc_masked_after_this_step * total_to_sample[track_name] + 0.1
        ).int()
        num_to_sample = still_masked - num_tokens_masked_after_this_step
        
        # Ensure num_to_sample is non-negative
        num_to_sample = max(0, num_to_sample.item())
        
        if num_to_sample == 0:
            continue
        
        # Apply sampling strategy
        strategy_applied = False
        if track_config.strategy == "entropy":
            track_entropy: torch.Tensor = getattr(entropy, track_name)
            if track_entropy is not None:
                track_entropy = track_entropy.to(device)
                
                if track_name == "function":
                    track_entropy = track_entropy.sum(-1)  # (B, L, D) -> (B, L)
                
                track_entropy = track_entropy.masked_fill(
                    ~sampling_mask, torch.finfo(track_entropy.dtype).max
                )
                _, indices = track_entropy.topk(min(num_to_sample, still_masked.item()), dim=-1, largest=False)
                is_top_k = torch.zeros((B, L), dtype=torch.bool, device=device).scatter(
                    1, indices, True
                )
                where_to_sample = sampling_mask & is_top_k
                strategy_applied = True
        # ======
        # NOTE: added stochastic strategy with Gumbel noise for better exploration
        elif track_config.strategy == "stochastic":
            track_entropy: torch.Tensor = getattr(entropy, track_name)
            if track_entropy is not None:
                track_entropy = track_entropy.to(device)

                if track_name == "function":
                    track_entropy = track_entropy.sum(-1)  # (B, L, D) -> (B, L)

                gumbel_noise = -torch.log(
                    -torch.log(torch.rand_like(track_entropy) + 1e-8) + 1e-8
                )
                noise_scale = torch.as_tensor(
                    perc_masked_after_this_step,
                    device=track_entropy.device,
                    dtype=track_entropy.dtype,
                )
                noised_track_entropy = track_entropy + gumbel_noise * noise_scale
                noised_track_entropy = noised_track_entropy.masked_fill(
                    ~sampling_mask, torch.finfo(noised_track_entropy.dtype).max
                )
                _, indices = noised_track_entropy.topk(
                    min(num_to_sample, still_masked.item()), dim=-1, largest=False
                )
                is_top_k = torch.zeros((B, L), dtype=torch.bool, device=device).scatter(
                    1, indices, True
                )
                where_to_sample = sampling_mask & is_top_k
                strategy_applied = True
        # ======
        
        if not strategy_applied or track_config.strategy == "random":
            _, masked_indices = sampling_mask.nonzero(as_tuple=True)
            if len(masked_indices) > 0:
                num_to_sample = min(num_to_sample, len(masked_indices))
                rnd_indices = masked_indices[torch.randperm(len(masked_indices))][
                    :num_to_sample
                ]
                rnd_mask = torch.zeros_like(sampling_mask)
                rnd_mask[:, rnd_indices] = True
                where_to_sample = sampling_mask & rnd_mask
            else:
                where_to_sample = sampling_mask & torch.zeros_like(sampling_mask)
        
        # Handle function track dimension expansion
        if track_name == "function":
            where_to_sample = where_to_sample.unsqueeze(-1).expand(
                B, L, tokenizers.function.depth
            )  # (B, L) -> (B, L, D)
        
        where_to_sample_dict[track_name] = where_to_sample
    
    return where_to_sample_dict


def iterative_sampling_multitrack_tokens(
    client: ESM3InferenceClient,
    input_tokens: list[ESMProteinTensor],
    configs: list[MultiTrackGenerationConfig],
    tokenizers: TokenizerCollectionProtocol,
) -> Sequence[ESMProteinTensor | ESMProteinError]:
    """Multi-track iterative sampling function.
    
    This function samples multiple tracks simultaneously in a single forward pass,
    with independent sampling parameters for each track. This is more efficient
    than calling iterative_sampling_tokens multiple times, as it requires only
    one forward pass per step instead of multiple.
    
    Args:
        client: ESM3 inference client
        input_tokens: List of input protein tensors
        configs: List of MultiTrackGenerationConfig objects (one per batch)
        tokenizers: Tokenizer collection
        
    Returns:
        List of sampled protein tensors or errors
    """
    devices = set([t.device for t in input_tokens])
    if len(devices) > 1:
        raise AttributeError(f"Input tokens on multiple devices {devices}")

    sampled_tokens = [attr.evolve(tokens) for tokens in input_tokens]

    # Clear structure tokens if user would like to condition only on coordinates.
    for tokens, config in zip(sampled_tokens, configs):
        if config.condition_on_coordinates_only and tokens.coordinates is not None:
            tokens.structure = None

    # Total sequence lengths.
    sequence_lengths = [len(tokens) for tokens in sampled_tokens]
    
    # Figure out the number of tokens to be sampled for each track in each prompt.
    # Structure: list of dicts, where each dict maps track_name -> num_tokens_to_sample
    total_to_sample_per_prompt = []
    
    for protein, config in zip(sampled_tokens, configs):
        total_to_sample_for_tracks = {}
        
        for track_name in config.get_all_tracks():
            track_tokens = getattr(protein, track_name)
            
            if track_tokens is None:
                # We need to sample the entire track.
                num_sampling_steps = _get_non_special_tokens(protein, tokenizers)
            else:
                masked = _get_masked_positions(
                    track_name, track_tokens, getattr(tokenizers, track_name).mask_token_id
                )
                num_sampling_steps = torch.sum(masked).item()
            
            total_to_sample_for_tracks[track_name] = num_sampling_steps
            
            # Adjust num_steps if over-specified
            track_config = config.get_track(track_name)
            if (num_sampling_steps > 0) and (num_sampling_steps < track_config.num_steps):
                track_config.num_steps = int(num_sampling_steps)
        
        total_to_sample_per_prompt.append(total_to_sample_for_tracks)

    # Get the maximum number of steps across all tracks and prompts.
    max_num_steps = max(
        [config.get_max_num_steps() for config in configs]
    )

    # Now stack the list to make a single batched ESMProteinTensor.
    # NOTE: This fills in None tracks with mask tokens
    batched_tokens = _stack_protein_tensors(
        sampled_tokens, sequence_lengths, tokenizers, devices.pop()
    )

    # ======
    # NOTE: added tracking of initial masks for allow_remask support
    # This enables DPLM2-style reparameterized decoding where positions can be reconsidered
    # IMPORTANT: Compute initial_masks AFTER batched_tokens is created, 
    # because _stack_protein_tensors fills None tracks with mask tokens
    # Structure: list of dicts, where each dict maps track_name -> initial_mask_tensor
    initial_masks_per_prompt = []
    for i, config in enumerate(configs):
        initial_masks_for_tracks = {}
        for track_name in config.get_all_tracks():
            track_config = config.get_track(track_name)
            
            if track_config.allow_remask:
                # Use batched_tokens which has masks filled in for None tracks
                track_data = getattr(batched_tokens, track_name)
                if track_data is not None:
                    # Get the i-th prompt's track data
                    prompt_track_data = track_data[i:i+1]  # Keep batch dimension
                    # Track which positions were initially masked for potential resampling
                    initial_mask = _get_masked_positions(
                        track_name, prompt_track_data, getattr(tokenizers, track_name).mask_token_id
                    )
                    initial_masks_for_tracks[track_name] = initial_mask
        
        initial_masks_per_prompt.append(initial_masks_for_tracks if initial_masks_for_tracks else None)
    # ======

    # Remember sampled prompts that has somehow errored out.
    errors: dict[int, ESMProteinError] = {}

    # Decode
    disable_tqdm = bool(os.environ.get("DISABLE_ITERATIVE_SAMPLING_TQDM", False))

    # ======
    # NOTE: CFG support for multi-track generation
    # Determine cfg_mode and whether CFG is enabled
    cfg_enabled = False
    is_cross_modal = False
    _active_cfg_mode = None
    for config in configs:
        for track_name in config.get_all_tracks():
            tc = config.get_track(track_name)
            if tc.cfg_scale > 0:
                cfg_enabled = True
                if tc.cfg_mode in ("cross_modal", "motif_anchored_cross_modal", "drop_motif_cross_modal"):
                    is_cross_modal = True
                if _active_cfg_mode is None:
                    _active_cfg_mode = tc.cfg_mode
    # ======

    for t in tqdm(range(max_num_steps), disable=disable_tqdm):
        # ======
        # NOTE: CFG support — batch cond+uncond in a single forward pass
        if cfg_enabled:
            uncond_result = _build_cfg_uncond_batched_tokens_multitrack(
                batched_tokens, configs, tokenizers, sequence_lengths,
                input_tokens=input_tokens,
                cfg_mode=_active_cfg_mode,
            )
            if is_cross_modal:
                uncond_for_seq, uncond_for_struct = uncond_result
                # 3×batch: [cond, uncond_for_seq, uncond_for_struct]
                combined = _BatchedESMProteinTensor()
                for f in attr.fields(ESMProteinTensor):
                    if f.name == "potential_sequence_of_concern":
                        continue
                    c = getattr(batched_tokens, f.name)
                    u1 = getattr(uncond_for_seq, f.name)
                    u2 = getattr(uncond_for_struct, f.name)
                    if c is not None and u1 is not None and u2 is not None:
                        setattr(combined, f.name, torch.cat([c, u1, u2], dim=0))
                    else:
                        setattr(combined, f.name, None)
                forward_out = _batch_forward(client, combined)
            else:
                uncond_batched_tokens = uncond_result
                combined_tokens = _cat_batched_tokens(batched_tokens, uncond_batched_tokens)
                forward_out = _batch_forward(client, combined_tokens)
        else:
            forward_out = _batch_forward(client, batched_tokens)
        B = len(configs)
        # ======

        # Sample each prompt individually, since their configuration may
        # be very different.
        for i, config in enumerate(configs):  # B
            if i in errors:
                # This prompt has errored out in previous steps.
                # Skip.
                continue

            per_prompt_cur_sampled = _BatchedESMProteinTensor.from_protein_tensor(
                batched_tokens.slice(i)
            )
            per_prompt_forward_out: LogitsOutput = _slice_tensor_dataclass(
                forward_out, i, keep_dim=True
            )

            # ======
            # NOTE: apply CFG to logits for multi-track generation
            if cfg_enabled:
                track_cfg_scales = {
                    tn: config.get_track(tn).cfg_scale
                    for tn in config.get_all_tracks()
                }
                has_any_cfg = any(s > 0 for s in track_cfg_scales.values())
                if has_any_cfg:
                    if is_cross_modal:
                        per_prompt_uncond_seq_out = _slice_tensor_dataclass(
                            forward_out, B + i, keep_dim=True
                        )
                        per_prompt_uncond_struct_out = _slice_tensor_dataclass(
                            forward_out, 2 * B + i, keep_dim=True
                        )
                        per_prompt_uncond_seq_out = _trim_sequence_tensor_dataclass(
                            per_prompt_uncond_seq_out, len(per_prompt_cur_sampled)
                        )
                        per_prompt_uncond_struct_out = _trim_sequence_tensor_dataclass(
                            per_prompt_uncond_struct_out, len(per_prompt_cur_sampled)
                        )
                        per_prompt_forward_out_trimmed = _trim_sequence_tensor_dataclass(
                            per_prompt_forward_out, len(per_prompt_cur_sampled)
                        )
                        per_prompt_forward_out = _apply_cfg_to_logits_cross_modal(
                            per_prompt_forward_out_trimmed,
                            per_prompt_uncond_seq_out,
                            per_prompt_uncond_struct_out,
                            track_cfg_scales,
                        )
                    else:
                        per_prompt_uncond_out = _slice_tensor_dataclass(
                            forward_out, B + i, keep_dim=True
                        )
                        per_prompt_uncond_out = _trim_sequence_tensor_dataclass(
                            per_prompt_uncond_out, len(per_prompt_cur_sampled)
                        )
                        per_prompt_forward_out_trimmed = _trim_sequence_tensor_dataclass(
                            per_prompt_forward_out, len(per_prompt_cur_sampled)
                        )
                        per_prompt_forward_out = _apply_cfg_to_logits_multitrack(
                            per_prompt_forward_out_trimmed,
                            per_prompt_uncond_out,
                            track_cfg_scales,
                        )
            # ======

            # Trim logits to proper sequence length for this prompt.
            per_prompt_forward_out = _trim_sequence_tensor_dataclass(
                per_prompt_forward_out,
                len(per_prompt_cur_sampled),
            )

            # Build SamplingConfig for all tracks
            sampling_config_dict = {}
            for track_name, track_config in config.tracks.items():
                # Handle temperature annealing for this track
                if track_config.temperature_annealing:
                    temperature = _get_annealed_temperature(
                        t, track_config.num_steps, track_config.temperature
                    )
                else:
                    temperature = track_config.temperature

                track_sample_config = SamplingTrackConfig()
                track_sample_config.invalid_ids = track_config.invalid_ids
                track_sample_config.temperature = temperature
                track_sample_config.top_p = track_config.top_p
                sampling_config_dict[track_name] = track_sample_config

            sampling_config = SamplingConfig(**sampling_config_dict)  # type: ignore

            # Sampling has to be done per-prompt, since sampling configs
            # are likely different for different prompts.
            # ======
            # NOTE: added allow_remask support for multi-track generation
            # Determine if allow_remask should be enabled for this call by checking if any track has it enabled
            allow_remask_for_call = any(
                config.get_track(track_name).allow_remask 
                for track_name in config.get_all_tracks()
            )
            # ======
            per_prompt_forward_and_sample_output = _sample_per_prompt(
                per_prompt_cur_sampled,
                per_prompt_forward_out,
                sampling_config,
                tokenizers,
                decode_sasa_tokens=False,
                allow_remask=allow_remask_for_call,
            )
            # ======

            # All positions sampled after _sample_per_prompt() above.
            per_prompt_new_sampled = per_prompt_forward_and_sample_output.protein_tensor

            # Find the positions we should sample this round for each track.
            assert per_prompt_forward_and_sample_output.entropy is not None
            try:
                where_to_sample_dict = _get_iterative_sampling_masks_for_multitrack(
                    per_prompt_cur_sampled,
                    torch.tensor(sequence_lengths[i]),
                    {
                        track_name: torch.tensor(total_to_sample_per_prompt[i][track_name])
                        for track_name in config.get_all_tracks()
                    },
                    t,
                    per_prompt_forward_and_sample_output.entropy,
                    config,
                    tokenizers,
                    initial_masks=initial_masks_per_prompt[i], # NOTE
                )
            except ValueError as e:
                errors[i] = ESMProteinError(error_code=500, error_msg=str(e))
                continue

            # Update batched_tokens for each track that was sampled
            for track_name, where_to_sample in where_to_sample_dict.items():
                where_to_sample = where_to_sample.to(input_tokens[0].device)
                
                old_track_samples = getattr(per_prompt_cur_sampled, track_name)
                new_track_samples = getattr(per_prompt_new_sampled, track_name)

                # ======
                # NOTE: added support for remasking already sampled positions when allow_remask is True
                track_config = config.get_track(track_name)
                initial_mask_for_track = (
                    initial_masks_per_prompt[i].get(track_name) 
                    if initial_masks_per_prompt[i] is not None 
                    else None
                )
                
                if track_config.allow_remask and (initial_mask_for_track is not None):
                    # get the is_mask for old_track_samples
                    is_mask = _get_masked_positions(
                        track_name, old_track_samples, getattr(tokenizers, track_name).mask_token_id
                    )
                    # if is_mask & where_to_sample, we sample from new_track_samples, else we keep old_track_samples
                    new_track_samples = torch.where(
                        where_to_sample & is_mask, new_track_samples, old_track_samples
                    )
                    # mask all positions that were not in where_to_sample
                    new_track_samples = torch.where(
                        where_to_sample, new_track_samples, getattr(tokenizers, track_name).mask_token_id
                    )
                    # protect special tokens (e.g. if BOS/EOS are not masked, they should never be sampled)
                    new_track_samples = torch.where(
                        initial_mask_for_track, new_track_samples, old_track_samples
                    )
                else:
                    # Iterative sampling by picking the tokens sampled this round
                    # from new_track_samples to old_track_samples.
                    new_track_samples = torch.where(
                        where_to_sample, new_track_samples, old_track_samples
                    )
                # ======

                # Update the corresponding row with new data.
                getattr(batched_tokens, track_name)[i, ...] = new_track_samples[0]

    # Un-pack to a list of single ProteinTypes.
    output_tokens = [
        batched_tokens.slice(i, sequence_len=sequence_lengths[i])
        if i not in errors
        else errors[i]
        for i in range(len(input_tokens))
    ]

    # Do not update tracks that were not sampled (e.g. keep None instead of masks)
    for inputs, outputs, config in zip(input_tokens, output_tokens, configs):
        if isinstance(outputs, ESMProteinError):
            continue

        # First restore coordinates field.
        # We know coordinates can never be iteratively sampled.
        setattr(outputs, "coordinates", getattr(inputs, "coordinates"))
        # Restore fields for tracks that were not sampled
        sampled_tracks = set(config.get_all_tracks())
        for f in attr.fields(ESMProteinTensor):
            if f.name == "coordinates":
                continue
            if f.name not in sampled_tracks:
                setattr(outputs, f.name, getattr(inputs, f.name))

    return output_tokens
# ======
