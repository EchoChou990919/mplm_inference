...
# NOTE: This file was modified from original file at DPLM-2
...
class MultimodalDiffusionProteinLanguageModel(nn.Module):
    ...
    def special_token_list(self):
        return [
            ...
            self.aa_O_id,
            self.aa_dot_id, # NOTE
            self.aa_dash_id, # NOTE
            self.aa_null1_id, # NOTE
        ]
    ...
    def _prepare_special_token(self):
        ...
        # ======
        # NOTE: added non-standard AA tokens to special token list 
        # to prevent them from being sampled during generation
        self.aa_dot_id = self.tokenizer._token_to_id["."]
        self.aa_dash_id = self.tokenizer._token_to_id["-"]
        self.aa_null1_id = self.tokenizer._token_to_id["<null_1>"]
        # =====
    ...
    def forward_decoder(
        ...
        # ====== 
        # NOTE: CFG support — split cfg_scale per track
        seq_cfg_scale=0.0,
        struct_cfg_scale=0.0,
        cfg_uncond_tokens=None,
        # ======
    ):
        ...
        # ======
        # NOTE: CFG batch forward with per-track cfg_scale
        any_cfg = (seq_cfg_scale > 0 or struct_cfg_scale > 0) and cfg_uncond_tokens is not None
        if any_cfg and isinstance(cfg_uncond_tokens, tuple):
            # cross_modal: 3×batch forward (cond + uncond_for_seq + uncond_for_struct)
            uncond_for_seq, uncond_for_struct = cfg_uncond_tokens
            stacked_tokens = torch.cat([output_tokens, uncond_for_seq, uncond_for_struct], dim=0)
            stacked_out = self.forward(input_ids=stacked_tokens)
            stacked_logits = stacked_out["logits"].log_softmax(dim=-1)
            cond_logits, uncond_seq_logits, uncond_struct_logits = stacked_logits.chunk(3, dim=0)
            # Per-position CFG: amplify cross-modal contribution
            logits = cond_logits.clone()
            if seq_cfg_scale > 0:
                guided = uncond_seq_logits + seq_cfg_scale * (cond_logits - uncond_seq_logits)
                logits[aa_position] = guided[aa_position]
            if struct_cfg_scale > 0:
                guided = uncond_struct_logits + struct_cfg_scale * (cond_logits - uncond_struct_logits)
                logits[struct_position] = guided[struct_position]
            attentions = stacked_out["attentions"] if need_attn_weights else None
            last_hidden_state = stacked_out["last_hidden_state"][:output_tokens.shape[0]]
        elif any_cfg:
            # non-cross_modal (drop_condition/drop_motif/full_mask): 2×batch forward
            stacked_tokens = torch.cat([output_tokens, cfg_uncond_tokens], dim=0)
            stacked_out = self.forward(input_ids=stacked_tokens)
            stacked_logits = stacked_out["logits"].log_softmax(dim=-1)
            cond_logits, uncond_logits = stacked_logits.chunk(2, dim=0)
            # Per-position CFG with separate scales
            logits = cond_logits.clone()
            if seq_cfg_scale > 0:
                guided = uncond_logits + seq_cfg_scale * (cond_logits - uncond_logits)
                logits[aa_position] = guided[aa_position]
            if struct_cfg_scale > 0:
                guided = uncond_logits + struct_cfg_scale * (cond_logits - uncond_logits)
                logits[struct_position] = guided[struct_position]
            attentions = stacked_out["attentions"] if need_attn_weights else None
            last_hidden_state = stacked_out["last_hidden_state"][:output_tokens.shape[0]]
        else:
            ...
            last_hidden_state = net_out["last_hidden_state"]
        # ======
    ...
    def _reparam_decoding(
        ...
        def _reparam_process(
            ...
            # ======
            # NOTE: added random top-k selection mode
            elif topk_mode == "random":
                pseudo_scores = torch.rand_like(_scores_for_topk)
                pseudo_scores = pseudo_scores.masked_fill(
                    ~non_special_sym_mask, 1000.0
                )
                lowest_k_mask = topk_masking(
                    pseudo_scores, cutoff_len, stochastic=False
                )
            # ======
            ...
            # when condition is 'uncond', the not_v1_t is equal to not_v2_t, the masked_to_noise is simply the lowest_k_mask
            # in output_tokens and output_scores, the lowest k positions will be updated with noise tokens and -inf scores
            ...
            # when condition is 'uncond', the masked_to_x0 is previously masked and not in the lowest k scores
            # for not the lowest k positions, previously masked -> updated by cur_tokens / cur_scores, previously unmasked -> remain unchanged
    ...
    def generate(
        ...
        for step in tqdm(range(max_iter), desc="Decoding"):
            # ====== 
            # NOTE: construct CFG unconditional tokens
            cfg_uncond_tokens = None
            any_cfg = seq_cfg_scale > 0 or struct_cfg_scale > 0
            if any_cfg and (partial_masks is not None or cfg_mode in ("cross_modal", "full_mask")):
                cfg_uncond_tokens = self._build_cfg_uncond_tokens(
                    prev_decoder_out["output_tokens"],
                    partial_masks=partial_masks,
                    cfg_mode=cfg_mode,
                )
            """
                any_cfg: whether to perform CFG guidance
                - partial_masks is not None: folding, inverse folding, and motif-scaffolding tasks provide partial_masks
                    - cfg_mode in ("drop_condition", "drop_motif", "cross_modal", "motif_anchored_cross_modal", "full_mask") are all applicable
                    - it is a bit broader than the actual reasonable modes, and we will not use all of them. 
                    - it doesn't matter much. we will set only the reasonable ones in the shell scripts.
                - partial_masks is None: unconditional cogeneration task does not provide partial_masks
                    - only "cross_modal" and "full_mask" modes are applicable
                    - "motif_anchored_cross_modal" requires partial_masks, so it is NOT applicable here
            """
            # ======
            ...
            with torch.no_grad():
                decoder_out = self.forward_decoder(
                    ...
                    # ====== 
                    # NOTE: pass CFG args — split cfg_scale
                    seq_cfg_scale=seq_cfg_scale,
                    struct_cfg_scale=struct_cfg_scale,
                    cfg_uncond_tokens=cfg_uncond_tokens,
                    # ======
                )
    ...
    # ====== 
    # NOTE: CFG unconditional token construction
    def _build_cfg_uncond_tokens(self, output_tokens, partial_masks=None, cfg_mode="drop_condition"):
        """Build unconditional tokens for CFG.

        Args:
            output_tokens: current tokens [B, 2L] with condition fixed and target partially decoded
            partial_masks: bool mask [B, 2L], True for condition positions (fixed).
                Required for drop_condition/drop_motif/motif_anchored_cross_modal, optional for cross_modal/full_mask.
            cfg_mode: how to construct unconditional input
                - "drop_condition": replace condition positions with mask tokens
                - "drop_motif": same as drop_condition (mask motif/condition positions)
                - "full_mask": replace ALL non-special positions with mask tokens
                - "cross_modal": return tuple (uncond_for_seq, uncond_for_struct)
                    uncond_for_seq: mask struct track (keep seq)
                    uncond_for_struct: mask seq track (keep struct)
                - "motif_anchored_cross_modal": like cross_modal but only mask scaffold positions
                ...
                - "drop_motif_cross_modal": drop_motif + cross_modal combined
                    uncond_for_seq: mask motif seq + ALL struct (guidance → motif seq faithfulness + cross-modal)
                    uncond_for_struct: mask motif struct + ALL seq (guidance → motif struct faithfulness + cross-modal)
        """
        type_ids = self.get_modality_type(output_tokens)
        non_special = self.get_non_special_symbol_mask(output_tokens)

        if cfg_mode in ("drop_condition", "drop_motif"):
            # Mask out the condition modality (the fixed/given input)
            # partial_masks is True where the condition is → replace those with mask tokens
            uncond_tokens = output_tokens.clone()
            aa_cond = partial_masks & type_ids.eq(self.aa_type) & non_special
            struct_cond = partial_masks & type_ids.eq(self.struct_type) & non_special
            uncond_tokens.masked_fill_(aa_cond, self.aa_mask_id)
            uncond_tokens.masked_fill_(struct_cond, self.struct_mask_id)
            return uncond_tokens
        elif cfg_mode == "cross_modal":
            # Two sets of uncond tokens for cross-modal guidance:
            # uncond_for_seq: mask struct track (remove struct info → for guiding seq)
            # uncond_for_struct: mask seq track (remove seq info → for guiding struct)
            uncond_for_seq = output_tokens.clone()
            uncond_for_struct = output_tokens.clone()
            struct_pos = type_ids.eq(self.struct_type) & non_special
            aa_pos = type_ids.eq(self.aa_type) & non_special
            uncond_for_seq.masked_fill_(struct_pos, self.struct_mask_id)
            uncond_for_struct.masked_fill_(aa_pos, self.aa_mask_id)
            return (uncond_for_seq, uncond_for_struct)
        elif cfg_mode == "motif_anchored_cross_modal":
            ...
            # partial_masks: True for motif/condition positions, False for scaffold positions.
            assert partial_masks is not None, "motif_anchored_cross_modal requires partial_masks"
            uncond_for_seq = output_tokens.clone()
            uncond_for_struct = output_tokens.clone()
            struct_pos = type_ids.eq(self.struct_type) & non_special
            aa_pos = type_ids.eq(self.aa_type) & non_special
            scaffold_struct = struct_pos & ~partial_masks  # scaffold struct positions only
            scaffold_aa = aa_pos & ~partial_masks          # scaffold seq positions only
            uncond_for_seq.masked_fill_(scaffold_struct, self.struct_mask_id)
            uncond_for_struct.masked_fill_(scaffold_aa, self.aa_mask_id)
            return (uncond_for_seq, uncond_for_struct)
        elif cfg_mode == "drop_motif_cross_modal":
            # Combine drop_motif + cross_modal:
            # uncond_for_seq: mask motif seq positions + mask ALL struct positions
            # uncond_for_struct: mask motif struct positions + mask ALL seq positions
            # Guidance direction: motif faithfulness + full cross-modal consistency
            assert partial_masks is not None, "drop_motif_cross_modal requires partial_masks"
            uncond_for_seq = output_tokens.clone()
            uncond_for_struct = output_tokens.clone()
            struct_pos = type_ids.eq(self.struct_type) & non_special
            aa_pos = type_ids.eq(self.aa_type) & non_special
            motif_aa = aa_pos & partial_masks
            motif_struct = struct_pos & partial_masks
            # uncond_for_seq: mask motif seq + ALL struct
            uncond_for_seq.masked_fill_(motif_aa, self.aa_mask_id)
            uncond_for_seq.masked_fill_(struct_pos, self.struct_mask_id)
            # uncond_for_struct: mask motif struct + ALL seq
            uncond_for_struct.masked_fill_(motif_struct, self.struct_mask_id)
            uncond_for_struct.masked_fill_(aa_pos, self.aa_mask_id)
            return (uncond_for_seq, uncond_for_struct)
        elif cfg_mode == "full_mask":
            # Mask ALL non-special positions (both condition and target)
            uncond_tokens = output_tokens.clone()
            aa_pos = type_ids.eq(self.aa_type) & non_special
            struct_pos = type_ids.eq(self.struct_type) & non_special
            uncond_tokens.masked_fill_(aa_pos, self.aa_mask_id)
            uncond_tokens.masked_fill_(struct_pos, self.struct_mask_id)
            return uncond_tokens
        else:
