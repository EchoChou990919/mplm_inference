...
class DPLM2Bit(DPLM2):
    ...
    def forward_decoder(
        ...
        if any_cfg and isinstance(cfg_uncond_tokens, tuple):
            ...
            cond_aa, uncond_seq_aa, uncond_struct_aa = stacked_out["aatype_logits"].chunk(3, dim=0)
            cond_st, uncond_seq_st, uncond_struct_st = stacked_out["struct_logits"].chunk(3, dim=0)
            # Seq logits: guide with uncond_for_seq (struct masked)
            if seq_cfg_scale > 0:
                aatype_logits = uncond_seq_aa + seq_cfg_scale * (cond_aa - uncond_seq_aa)
            else:
                aatype_logits = cond_aa
            # Struct logits: guide with uncond_for_struct (seq masked)
            if struct_cfg_scale > 0:
                struct_logits = uncond_struct_st + struct_cfg_scale * (cond_st - uncond_struct_st)
            else:
                struct_logits = cond_st
            attentions = stacked_out["attentions"] if need_attn_weights else None
            all_hidden_states = stacked_out["all_hidden_states"][:output_tokens.shape[0]] if stacked_out.get("all_hidden_states") is not None else None
        elif any_cfg:
            ...
            stacked_out = self.forward(input_ids=stacked_tokens)
            cond_aa_logits, uncond_aa_logits = stacked_out["aatype_logits"].chunk(2, dim=0)
            cond_struct_logits, uncond_struct_logits = stacked_out["struct_logits"].chunk(2, dim=0)
            # Per-track CFG with separate scales
            if seq_cfg_scale > 0:
                aatype_logits = uncond_aa_logits + seq_cfg_scale * (cond_aa_logits - uncond_aa_logits)
            else:
                aatype_logits = cond_aa_logits
            if struct_cfg_scale > 0:
                struct_logits = uncond_struct_logits + struct_cfg_scale * (cond_struct_logits - uncond_struct_logits)
            else:
                struct_logits = cond_struct_logits
            attentions = stacked_out["attentions"] if need_attn_weights else None
            all_hidden_states = stacked_out["all_hidden_states"][:output_tokens.shape[0]] if stacked_out.get("all_hidden_states") is not None else None
        else:
