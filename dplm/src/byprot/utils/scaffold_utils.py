...
def get_initial_dplm2(args, aa_seq, struct_seq, tokenizer, pdb, ori_pdb, device):
    ...
    # ======
    # NOTE: add option for collate by length or not
    if args.batch_size > 0:
        ...
        batches = collate_by_length(tokenizer, init_aa_seq, init_struct_seq, args, device)
        start_idxs_list = []
        end_idxs_list = []
        for batch in batches:
            start_idxs, end_idxs = create_idxs_list(pdb, tokenizer, batch, args)
            start_idxs_list += start_idxs
            end_idxs_list += end_idxs
    # ======
...
# ======
# NOTE: add collate by length function for DPLM-2 motif-scaffolding
def collate_by_length(tokenizer, init_aa_seq, init_struct_seq, args, device):
    """Groups sequences by length and creates a batch for each length group.
    
    Args:
        tokenizer: Tokenizer for encoding sequences
        init_aa_seq: List of amino acid sequences
        init_struct_seq: List of structure sequences
        args: Arguments containing num_seqs
        device: Device to put tensors on
        
    Returns:
        batches: List of batches, each containing sequences of the same length
    """
    # Create a mapping from length to indices
    length_to_indices = {}
    for idx, aa_seq in enumerate(init_aa_seq):
        # Calculate the actual token count after tokenization
        seq_token_ids = tokenizer(aa_seq, add_special_tokens=False, padding=False)["input_ids"]
        iter_steps = len(seq_token_ids) // args.maxiter_seqlen_ratio
        if iter_steps not in length_to_indices:
            length_to_indices[iter_steps] = []
        length_to_indices[iter_steps].append(idx)
    
    batches = []
    
    # Process each length group
    for seq_len in sorted(length_to_indices.keys()):
        indices = length_to_indices[seq_len]
        
        # Extract sequences for this length group
        group_aa_seq = [init_aa_seq[i] for i in indices]
        group_struct_seq = [init_struct_seq[i] for i in indices]
        
        # Encode amino acid sequences
        batch_aa = tokenizer.batch_encode_plus(
            group_aa_seq, add_special_tokens=False, padding="longest", return_tensors="pt"
        )
        batch_aa = {
        ...
        # Combine amino acid and structure batches
        batch = {
            "input_ids": torch.cat(
        ...
        # Create type_ids: 0 for struct, 1 for aa, 2 for padding
