...
def initialize_conditional_generation(
    ...
    # ====== 
    # NOTE: batch_size == 0 means all samples in one batch 
    # then batch_size < 0 means group samples by iter steps (sequence length // maxiter_seqlen_ratio)
    elif args.batch_size == 0:
        batches = [build_batch(input_data_aatype, input_data_struct_tokens)]
        input_data_name_list = [input_data_name]
    else:
        # Group by iter steps (sequence length // maxiter_seqlen_ratio)
        iter_step_groups = defaultdict(list)
        if args.task == "folding":
            for i, seq in enumerate(input_data_aatype):
                iter_step = int((len(seq) - 16) // args.maxiter_seqlen_ratio)
                iter_step_groups[iter_step].append(i)
        elif args.task == "inverse_folding":
            for i, seq in enumerate(input_data_struct_tokens):
                iter_step = int((len(seq) - 24) / 4 // args.maxiter_seqlen_ratio)
                iter_step_groups[iter_step].append(i)
        else:
            raise NotImplementedError
        for indices in iter_step_groups.values():
            input_data_aa_batch = [input_data_aatype[i] for i in indices]
            input_data_struct_batch = [input_data_struct_tokens[i] for i in indices]
            new_batch = build_batch(
                input_data_aa_batch, input_data_struct_batch
            )
            batches.append(new_batch)
            input_data_name_list.append([input_data_name[i] for i in indices])
    # ======
...
def unconditional_generate(args):
    ...
    for seq_len in args.seq_lens:
        # ======
        # NOTE: adjust max_iter based on max_iter and sequence length
        if args.max_iter < 0:
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            for input_tokens in input_tokens_batch:
                ...
                outputs = model.generate(
                    ...
                    # ====== 
                    # NOTE: CFG support for unconditional generation
        ...
        save_results(
            ...
            save_dir=os.path.join(args.saveto, f"length_{seq_len}"),
            tokenizer=tokenizer,
...
def conditional_generate_from_fasta(args):
    ...
    for i, batch in enumerate(tqdm(batches, desc=f"{args.task}")):
        ...
        if args.batch_size < 0:
            seq_len = batch["partial_mask"].sum(dim=1)
            max_iter = int(seq_len.max().item() // args.maxiter_seqlen_ratio)
        else:
            max_iter = args.max_iter
        # ======
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
...
def main():
    ...
    # ======
    # NOTE: add maxiter_seqlen_ratio and batch_size
    ...
    parser.add_argument("--no_save_pdb", action="store_false", dest="save_pdb")
    parser.add_argument("--bit_model", action="store_true")

    # ====== 
    # NOTE: CFG options — split cfg_scale per track
    parser.add_argument("--cfg_scale", type=float, default=None,
                        help="CFG scale alias. If set, overrides both seq/struct cfg_scale.")
    ...
    parser.add_argument("--cfg_mode", type=str, default="drop_condition",
    ...
    # ======
    # NOTE: backward compat for --cfg_scale alias
    if args.cfg_scale is not None and args.cfg_scale > 0:
        args.seq_cfg_scale = args.cfg_scale
        args.struct_cfg_scale = args.cfg_scale
    # ======
